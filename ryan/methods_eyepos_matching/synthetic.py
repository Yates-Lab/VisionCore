...
def make_session(kinds, n_trials=200, n_phases=100, sigma_eye=0.15,
    ...
    n_trials, n_phases : int
        Repeats and frozen-stimulus phases (time bins).
    sigma_eye : float
    ...
    # Zero-mean phase-locked PSTH drive, one profile per cell.
    ...
    rate = np.empty((n_trials, n_phases, n_cells))
    ...
    # cache the close-pair (p2) eye-sampler parameters so callers can draw the
    # exact close-pair distribution N(0, sigma_eye^2/2 I) when needed.
    out_sd_p2 = sigma_eye / np.sqrt(2.0)
    _ = out_sd_p2  # documented invariant; not returned to keep the dict lean
