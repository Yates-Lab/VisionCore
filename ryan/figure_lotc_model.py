...
sys.path.append('./VisionCore/scripts')
from figure_common import tee_to_file
import numpy as np
...
import dill
with open(f'dmcfarland_outputs_{mode}.pkl', 'rb') as f:
    outputs = dill.load(f)
with open(f'dmcfarland_analyzers_{mode}.pkl', 'rb') as f:
...
fig_dir = Path('./figures/mcfarland')
...
for i in range(len(metrics[0]['Ctotal'])):
    ...
    ax.axis('off')
    fig.savefig(fig_dir / f'covariance_decomposition_{i}_{window_idx}_psth.pdf', bbox_inches='tight', dpi=300) 
    ...
    ax.axis('off')
    fig.savefig(fig_dir / f'covariance_decomposition_{i}_{window_idx}_full.pdf', bbox_inches='tight', dpi=300) 
...
fig.savefig(fig_dir / "fano_paper_figure.pdf", bbox_inches="tight", dpi=300)

fig, axs = plot_supp_var_mean_panels(metrics, savepath=fig_dir / "var_mean_panels.pdf")
fig, axs = plot_supp_fano_hist_panels(metrics, savepath=fig_dir / "fano_hist_panels.pdf")
...
fig.savefig(fig_dir / "noisecorr_paper_figure.pdf", bbox_inches="tight", dpi=300)

# supplements
plot_supp_noisecorr_hist_panels(metrics, savepath=fig_dir / "supp_noisecorr_hist_panels.pdf")
plot_supp_noisecorr_shuffle_delta_panels(metrics, savepath=fig_dir / "supp_noisecorr_shuffle_delta_panels.pdf")
plot_supp_noisecorr_2d_panels(metrics, savepath=fig_dir / "supp_noisecorr_2d_panels.pdf")
plot_supp_noisecorr_shuffle_corrected_panels(metrics, savepath=fig_dir / "supp_noisecorr_shuffle_corrected_panels.pdf")
...
fig.savefig(fig_dir / "alpha_paper_figure.pdf", bbox_inches="tight", dpi=300)

# supplemental
plot_supp_alpha_hist_panels(metrics, savepath=fig_dir / "supp_alpha_hist_panels.pdf")
plot_supp_alpha_shuffle_comparison(metrics, alpha_stats, savepath=fig_dir / "supp_alpha_shuffle_null_panels.pdf")

plot_supp_alpha_vs_metrics(metrics, which="uncorr", savepath=fig_dir / "supp_ffuncorr_vs_1minusalpha.pdf")
plot_supp_alpha_vs_metrics(metrics, which="corr",   savepath=fig_dir / "supp_ffcorr_vs_1minusalpha.pdf")
plot_supp_alpha_vs_metrics(metrics, which="delta",  savepath=fig_dir / "supp_deltaff_vs_1minusalpha.pdf")
...
fig.savefig(fig_dir / "subspace_main.pdf", bbox_inches="tight", dpi=300)

fig, _ = plot_subspace_vs_shuffle_figure(S, title="Subspace alignment vs chance")
fig.savefig(fig_dir / "subspace_vs_shuffle_supp.pdf", bbox_inches="tight", dpi=300)
...
fig_main, axs_main = create_main_figure(
    ...
    figsize=(11, 9),
    savepath=fig_dir / "main_figure_CK.pdf",
)
