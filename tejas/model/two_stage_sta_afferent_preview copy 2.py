...
def save_combined_preview(
    ...
    sta_exc = np.clip(sta_rf, a_min=0.0, a_max=None)
    sta_inh = np.clip(-sta_rf, a_min=0.0, a_max=None)
    sta_joint_abs = np.concatenate([sta_exc.reshape(-1), sta_inh.reshape(-1)])
    sta_amp_scale = float(np.percentile(sta_joint_abs, 99)) if sta_joint_abs.size else 1.0
    sta_carrier_scale = float(sta_joint_abs.max()) if sta_joint_abs.size else 1.0
    sta_exc_rgb = render_energy_component_rgb(
        sta_exc, hue_rgb=(0.95, 0.70, 0.35), amp_scale=sta_amp_scale, carrier_scale=sta_carrier_scale
    )
    sta_inh_rgb = render_energy_component_rgb(
        sta_inh, hue_rgb=(0.45, 0.70, 0.95), amp_scale=sta_amp_scale, carrier_scale=sta_carrier_scale
    )

    sta_fig, axes = plt.subplots(2, 4, figsize=(16, 8))
    axes[0, 0].imshow(model_lin, cmap="coolwarm_r")
    axes[0, 0].set_title("Linear RF")
    axes[0, 0].axis("off")
    axes[0, 1].imshow(model_exc_rgb)
    axes[0, 1].set_title("Energy Exc RF")
    axes[0, 1].axis("off")
    axes[0, 2].imshow(model_inh_rgb)
    axes[0, 2].set_title("Energy Inh RF")
    axes[0, 2].axis("off")
    axes[0, 3].imshow(sta_rf, cmap="coolwarm_r")
    axes[0, 3].set_title(f"STA (cell {cell_id})")
    axes[0, 3].axis("off")

    axes[1, 0].imshow(sta_rf, cmap="coolwarm_r")
    axes[1, 0].set_title("Actual STA RF")
    axes[1, 0].axis("off")
    axes[1, 1].imshow(sta_exc_rgb)
    axes[1, 1].set_title("Actual STA Energy Exc")
    axes[1, 1].axis("off")
    axes[1, 2].imshow(sta_inh_rgb)
    axes[1, 2].set_title("Actual STA Energy Inh")
    axes[1, 2].axis("off")
    axes[1, 3].axis("off")
    axes[1, 3].text(
        0.5,
        0.5,
        "Top row: training-style model diagnostics\nBottom row: raw STA decomposition",
        ha="center",
        va="center",
        fontsize=10,
    )
...
def main():
    ...
    local_idx = cell_ids.index(args.example_cell_id)
    lag_idx = lag_slots[local_idx]

    out_dir = Path(args.output_dir)
    out_dir.mkdir(parents=True, exist_ok=True)
    out_png = out_dir / f"sta_afferent_cell_{args.example_cell_id:03d}.png"
    save_combined_preview(
        model=model,
        stas=info["stas"],
        cell_id=args.example_cell_id,
        lag_idx=lag_idx,
