...
def save_cell_best_png(
    ...
    show_epoch_diagnostics(
        ...
        save_prefix=f"cell_{int(cell_id):03d}_best",
        close_figs=True,
        show_plots=False,
    )
...
for prox_mult in prox_mults:
    for local_mult in local_mults:
        ...
        for lag_index, group_cell_ids in sorted(lag_to_cells.items()):
            print(f"[group] lag={lag_index} n_cells={len(group_cell_ids)}")
            ...
            for epoch in range(num_epochs):
                ...
                step_stats, _ = train_step_lbfgs(
                    ...
                    batch=None,
                    batch_loader=train_loader_lbfgs_accum,
                ...
                for batch in tqdm(train_loader_lbfgs_accum):
                    model.eval()
                    batch = prepare_batch_one_lag(
                        batch=batch,
                        lag_index=lag_index,
