...
def plot_robs(robs, iix, cc, num_psth = None, distances_along_line = None, alpha_raster =1):
        ...
        if num_psth is not None:
            ...
            with warnings.catch_warnings():
                ...
                psths_all = [np.nanmean(robs[iix[i*num_indices_for_each_psth:(i+1)*num_indices_for_each_psth],:,cc], axis=0) for i in range(num_psth)]
        ...
        ax1.imshow(robs_original[np.sort(iix),:,cc], alpha=alpha_raster, aspect='auto', cmap = "gray_r")
        ...
        if num_psth is not None:
            ...
            with warnings.catch_warnings():
                warnings.simplefilter("ignore", RuntimeWarning)
                psths = [np.nanmean(robs_original[iix,:,cc], axis=0) for i in range(num_psth)]
            with warnings.catch_warnings():
                warnings.simplefilter("ignore", RuntimeWarning)
                psths_ste = [np.nanstd(robs_original[iix,:,cc], axis=0) / np.sqrt(num_indices_for_each_psth) for i in range(num_psth)]
        ...
        ax1 = axes[0][1]
        ax1.imshow(robs[iix,:,cc], alpha=alpha_raster, aspect='auto', cmap = "gray_r")
        ...
        if num_psth is not None:
            ...
            with warnings.catch_warnings():
                warnings.simplefilter("ignore", RuntimeWarning)
                psths = [np.nanmean(robs[iix[i*num_indices_for_each_psth:(i+1)*num_indices_for_each_psth],:,cc], axis=0) for i in range(num_psth)]
                psths_ste = [np.nanstd(robs[iix[i*num_indices_for_each_psth:(i+1)*num_indices_for_each_psth],:,cc], axis=0) / np.sqrt(len(iix[i*num_indices_for_each_psth:(i+1)*num_indices_for_each_psth])) for i in range(num_psth)]
...
# for cc, peak_times in cids_and_peak_times:
# for cc in units_qc:
# for cc, _ in cids_and_peak_times:
...
# 154 from Allen_2022-04-15 and 158 from Allen_2022-04-06 are best
    ...
    def display(max_orientation):
        ...
        for i in range(total_start_time, total_end_time, len_of_each_segment):
            ...
            psth = np.nanmean(robs[:, :, cc], axis=0)
