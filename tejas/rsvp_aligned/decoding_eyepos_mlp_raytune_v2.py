#!/usr/bin/env python3
"""
Ray Tune hyperparameter optimization for MLP eyepos decoding model (v2).
Runs 500 trials: first 50 random, then OptunaSearch with TPESampler.
Trains for 200 epochs and tracks best val_loss and epoch.
"""
import os
...
FIXED_HYPERPARAMS = {
    ...
    'lag_bins': 0,
    'num_epochs': 200,  # Fixed at 200 epochs
    'learning_rate': 1e-3,
    'weight_decay': 0,  # Not used with schedulefree.RAdamScheduleFree
...
robs_aligned, eyepos_aligned = apply_lag(robs, eyepos, FIXED_HYPERPARAMS['lag_bins'])
...
def train_mlp(config):
    ...
    # Time encoding
    time_encoding = None
    if config['use_time_encoding'] and config['time_enc_dim'] > 0:
        time_encoding = build_time_encoding(time_len, int(config['time_enc_dim']))
        time_encoding = np.tile(time_encoding[None, :, :], (PREPROCESSED_DATA['num_trials'], 1, 1))
    
    # Standardize inputs (fixed to False for now, but can be tuned if needed)
    standardize_inputs = False  # Not in tuned list
    ...
    if time_encoding is not None:
        robs_feat_model = np.concatenate(
            [robs_z, config['time_enc_scale'] * time_encoding], axis=2
        )
    else:
    ...
    if config['use_image_id_encoding']:
        ...
        ids = PREPROCESSED_DATA['image_ids_reference'][
            PREPROCESSED_DATA['time_window_start']:PREPROCESSED_DATA['time_window_end'] - FIXED_HYPERPARAMS['lag_bins']
        ].astype(int)
        valid_mask = ids >= 0
    ...
    # Create datasets
    # Use "cuda" (not "cuda:0") to let Ray manage device assignment with fractional GPUs
    ...
    train_dataset = WindowedEyeposDataset(
        ...
        int(config['window_stride']), FIXED_HYPERPARAMS['min_valid_fraction'],
        device, FIXED_HYPERPARAMS['cache_data_on_gpu'],
        augment=True,
    ...
    loader_num_workers = 0 if FIXED_HYPERPARAMS['cache_data_on_gpu'] else FIXED_HYPERPARAMS['dataloader_num_workers']
    loader_pin_memory = False if FIXED_HYPERPARAMS['cache_data_on_gpu'] else FIXED_HYPERPARAMS['dataloader_pin_memory']
    ...
    # Training loop
    num_epochs = FIXED_HYPERPARAMS['num_epochs']  # 200 epochs
    ...
    for epoch in range(num_epochs):
        ...
        for X_batch, y_batch, mask_batch, time_idx in train_loader:
            ...
            if FIXED_HYPERPARAMS['use_trajectory_loss']:
...
# Define search space (only the specified hyperparameters)
search_space = {
    # Time encoding
    'use_time_encoding': tune.choice([True, False]),
    'time_enc_dim': tune.choice([4, 8, 12, 16, 20, 24, 32]),  # Must be even
    'time_enc_scale': tune.loguniform(0.1, 10.0),
    
    # Window and data
    'window_stride': tune.choice([1, 2, 3, 5, 10]),
    'batch_size': tune.choice([16, 32, 64, 128, 256]),
    'center_per_trial': tune.choice([True, False]),
    
    # Augmentation
    'sample_poisson': tune.choice([True, False]),
    'augmentation_neuron_dropout': tune.uniform(0.0, 1.0),
    'augmentation_turn_off_percentage': tune.uniform(0.0, 1.0),
    'augmentation_turn_on_percentage': tune.uniform(0.0, 1.0),  # Added since turn_off was listed twice
    'augmentation_mixup_alpha': tune.uniform(0.0, 1.0),
    ...
    'mlp_dropout_rate': tune.uniform(0.0, 1.0),
...
# Configure Ray Tune
scheduler = ASHAScheduler(metric="val_loss", mode="min", max_t=200, grace_period=20)

# Configure OptunaSearch with TPESampler
# First 50 trials will be random (n_startup_trials=50), then TPE takes over
tpe_sampler = optuna.samplers.TPESampler(
    n_startup_trials=50,  # 50 fully random trials, then TPE from trial #51 on
...
# Configure resources - allow multiple trials to share GPU 0
# Use fractional GPU allocation: 1/20 = 0.05 GPU per trial allows 20 concurrent trials on 1 GPU
trainable_with_resources = tune.with_resources(
    train_mlp,
    resources={"gpu": 0.05, "cpu": 3}  # Fractional GPU allows multiple trials to share GPU 0
)
...
tuner = tune.Tuner(
    ...
    tune_config=tune.TuneConfig(
        num_samples=500,  # 500 total trials
        scheduler=scheduler,
        search_alg=search_alg,
        max_concurrent_trials=20,  # 20 concurrent trials
    ),
    run_config=tune.RunConfig(
        name="mlp_eyepos_tune_v2",
        storage_path=os.path.abspath("./ray_results"),
        stop={"training_iteration": 200},  # Max epochs
    ),
)
...
if __name__ == "__main__":
    ...
    try:
        ray.init(
            ...
            num_cpus=60,  # Allow enough CPUs for 20 concurrent trials (3 CPUs per trial)
    ...
    # Run tuning
    print("Starting Ray Tune hyperparameter optimization (v2)...")
    print(f"Total trials: 500 (first 50 random, then OptunaSearch with TPESampler)")
    print(f"Concurrent trials: 20")
    print(f"Using GPU 0 only (0.05 GPU per trial)")
    print(f"Training for 200 epochs per trial")
    print("="*80)
    ...
    try:
        ...
        print("="*80)
        best_val_loss = best_result.metrics.get('best_val_loss', best_result.metrics.get('val_loss', 'N/A'))
        best_epoch = best_result.metrics.get('best_epoch', 'N/A')
