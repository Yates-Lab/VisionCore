...
use_2d_attention = True  # 2d_attention flag (axial time x neuron)
...
class CNNEyepos(torch.nn.Module):
    def __init__(self, input_dim, num_filters=64, num_layers=2, dropout_rate=0.1):
        super().__init__()
        # Input: (batch, time, features)
        # Convert to (batch, features, time) for 1D conv
        self.conv_layers = torch.nn.ModuleList()
        self.conv_layers.append(
            torch.nn.Sequential(
                torch.nn.Conv1d(input_dim, num_filters, kernel_size=5, padding=2),
        ...
        for _ in range(num_layers - 1):
            self.conv_layers.append(
                torch.nn.Sequential(
                    torch.nn.Conv1d(num_filters, num_filters, kernel_size=3, padding=1),
                    torch.nn.BatchNorm1d(num_filters),
                    torch.nn.ReLU(),
                    torch.nn.Dropout(dropout_rate)
                )
            )
        self.head = torch.nn.Conv1d(num_filters, 2, kernel_size=1)
