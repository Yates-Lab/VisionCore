...
from eval.eval_stack_utils import load_single_dataset, get_stim_inds, evaluate_dataset, scan_checkpoints
from eval.eval_stack_multidataset import load_model, load_single_dataset
...
models_by_type = scan_checkpoints(checkpoint_dir, verbose=True)

#%%

print(f"Found {len(models_by_type)} model types:")
for model_type, models in models_by_type.items():
    best_loss = models[0]['val_bps'] if models else 'N/A'
...
stim_type = 'fixrsvp'
bps_rescaled = bits_per_spike(bps_results[stim_type]['rhat'], bps_results[stim_type]['robs'], bps_results[stim_type]['dfs'])

plt.plot(bps_rescaled.detach().cpu().numpy(), label='Rescaled')
plt.axhline(0)
plt.ylim(-1, 2)
#%%


bps_results[stim_type]['dfs'].shape
...
#%%
cc += 1
_ = plt.plot(ccnorm_results['rbar'][:,cc])
_ = plt.plot(ccnorm_results['rbarhat'][:,cc])
