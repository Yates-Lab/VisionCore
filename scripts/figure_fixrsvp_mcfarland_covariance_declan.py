...
#%% Load saved Luke McFarland results
import pickle
...
# 4. Check Rank of the last window
window_idx = -1
...
plt.figure()
plt.plot(s, 'o-')
plt.title(f"Singular Values of FEM Covariance ({win_ms}ms)")
plt.show()
...
plt.figure()
plt.plot(s, 'o-')
plt.title(f"Singular Values of Total Covariance ({win_ms}ms)")
plt.show()
...
plt.figure()
plt.plot(s, 'o-')
plt.title(f"Singular Values of Noise Correlation ({win_ms}ms)")
plt.show()
# %%
i = 3
...
plt.title(f"FF Window Size ({results[i]['window_ms']}ms)")
