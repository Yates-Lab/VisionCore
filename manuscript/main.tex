...
% Coauthor review: revised wording and the new reference are shown in red.
\usepackage{etoolbox}
\DeclareRobustCommand{\reviewedit}[1]{{\color{red}#1}}
\pdfstringdefDisableCommands{\def\reviewedit#1{#1}}
\AtBeginEnvironment{thebibliography}{%
  \let\revieworiginalbibitem\bibitem
  \renewcommand{\bibitem}[2][]{%
    \ifstrequal{#2}{assa2026temporal}{\color{red}}{\ifstrequal{#2}{lindsay2018attention}{\color{red}}{\color{black}}}%
    \revieworiginalbibitem[#1]{#2}%
  }%
}
...
\begin{center}
\vspace{-0.25em}
{\large Ryan Ressmeyer$^{1*}$, Declan P. Rowley$^{2*}$, Tejas Kothapalli$^{3,4*}$, Dekel Galor$^{3,4}$, Ruei-Jr Wu$^{5}$, Michele Rucci$^{5}$, Jude F. Mitchell$^{5}$, Daniel Butts$^{6}$, Alexander C. Huk$^{2}$, Jacob L. Yates$^{3,4}$\par}
...
In natural settings, sensation is inseparable from action. An animal's own movements continually shape the signals arriving at its sensory receptors. Neural activity therefore depends not only on external stimuli but also on the animal's recent movement history (the sensory consequences of these movements are termed ``reafference''). Vision provides an unusually precise example of this coupling between sensation and action. The eyes are never at rest \cite{Jurin1738, martinez2017unchanging}, and fixational eye movements (FEMs)---including drift and microsaccades---continually translate the image across the retina. Decades of psychophysical work show that these minute displacements support fine spatial vision \cite{rucci2007miniature, ratnam2017benefits, intoy2020finely}. Retinal motion converts spatial structure into temporal modulation. Theoretical work treats this temporal signal as a carrier of spatial information \cite{rucci2015unsteady, rucci2018temporal, ahissar2012seeing} that can be read out to resolve detail finer than the cone mosaic alone would permit \cite{anderson2020high, assa2026temporal}. Consistent with this account, neurophysiological studies have found substantial effects of FEMs in the retina \cite{wu2024fixational} and primary visual cortex (V1) \cite{snodderly2001selective, kagan2008saccades, baudot2013animation, mcfarland2016variability}. Yet how this self-generated retinal motion shapes visual encoding across cortical populations remains unknown.

Answering this question requires determining how much cortical activity varies with FEMs and how those variations relate to the response shared across repeated presentations of the same displayed stimulus. Conventional analyses often assume that these presentations deliver the same sensory input, attributing trial-to-trial differences in response to internally generated variability \cite{tolhurst1983statistical, shadlen1998variable}. During fixation, this assumption can fail because FEMs dynamically change the retinal image even when the displayed stimulus is repeated \cite{mcfarland2016variability}. Analyses that do not account for changes in the eye's position therefore risk attributing differences in retinal input to intrinsic variability of the neural circuit. Separating these components is necessary to interpret estimates of response reliability and the mechanisms underlying that reliability \cite{tolhurst1983statistical, softky1993highly, shadlen1998variable, gur1997response}.

The effects of FEMs are expected to be especially consequential for foveal vision. The primate fovea subtends the central $\sim$5--8 degrees of visual angle across species \cite{franco2000conservation}, supports the highest spatial acuity, and is disproportionately represented in V1 and other cortical areas \cite{fritsches1996visuotopic}. FEMs can displace the retinal image by amounts comparable to the receptive-field sizes of individual foveal neurons. To understand how these displacements shape the cortical activity supporting fine spatial vision, we used laminar electrode probes to record spiking activity from ensembles of neurons in the foveal representation of marmoset V1 during alert fixation of naturalistic images. Relating this activity to retinal input required measuring eye position with high precision, which we achieved using a custom digital dual Purkinje image eye tracker \cite{yates2023detailed, wu2023high, ressmeyer2025openirisdpi}. We repeatedly presented a fixed sequence of naturalistic images while recording cortical responses and the eye's trajectory on each presentation. Comparing neural responses with respect to the stimiliarity of the eye's trajectory between repeats allowed us to separate trial-to-trial variability associated with FEMs from variability that remained unexplained.

FEMs accounted for approximately 70\% of the conditional firing-rate variance in individual neurons, exceeding the contribution captured by the trial-averaged response. Accounting for the effects of FEMs also substantially reduced conventional estimates of residual variability (``noise''), both independent and shared between units. FEM-driven variance was concentrated in a few dominant population dimensions that overlapped those of the trial-averaged response. A deep neural network encoding model predicted much of the single-trial modulation from retinal input, with a much smaller additional contribution from explicit eye-position and eye-velocity inputs, suggesting that feedforward visual processing is sufficient to account for the dependence of activity on FEMs.

In the model, FEMs increased spatial information in select neural subpopulations compared with stabilized retinal input. These effects depended on eye-movement dynamics, stimulus content, and neuronal spatiotemporal tuning. Combining contributions from successive nonlinear processing stages increased spatial sharpening in the model, suggesting a nonlinear contribution to the information improvements. Together, these results demonstrate the fundamental role of active sensing in foveal vision, whereby fine-scale eye movements drive large changes in cortical ensemble activity while sharpening the representation of spatial features. Reafference has been demonstrated as a key element of active sensing in other sensory modalities; our results show that it also plays a central role in the highest-acuity form of vision.

\section{Results}

\subsection{Fixational eye movements exert large effects on the structure of neural responses in foveal V1}
...
\begin{figure}[p]
 ...
 \includegraphics[width=\columnwidth]{figures/figure1.pdf} 
    ...
    \textbf{(A)} Schematic of the experimental paradigm. A head-fixed marmoset fixated a rapidly updating sequence of flashed images ($3^\circ$ diameter). Electrical activity was recorded using laminar probes from the lateral surface of V1. The eye's position was tracked using a high-precision digital dual Purkinje image eye tracker \cite{wu2023high, ressmeyer2025openirisdpi, yates2023detailed}. The eye was imaged through a hot mirror that allowed the subject to view the stimulus. A representative image of the Purkinje images is shown in the inset.
    ...
    \textbf{(C)} Receptive field (RF) locations. The extent of each unit's RF is drawn as a contour. Blue contours represent an ensemble of units recorded from monkey A; green contours show an example ensemble from monkey L. All units had receptive fields within $1^\circ$ from the fovea.} % Optional marker
    \label{fig:fig1}
\end{figure}
...
\begin{figure}[!t]
    ...
    \textbf{(E)} Left: The RF for the unit shown in (D), which was obtained by spike-triggered averaging. The RF displays clear, Gabor-like structure. Right: The average eye position in a 50 ms window across trials. Points are colored according to their projection onto the line orthogonal to the subunits of the RF. 
    \textbf{(F)} Trial rasters shown in (D), now ordered by the projection of the mean eye position across the RF in 50 ms windows. Vertical gray lines denote the window boundaries. Smooth variation in trial-to-trial responses emerges when sorting by eye position, indicating a relation between the eye's position and the neural response. 
...
We recorded from populations of 10--118 analysis-eligible units per session (1,022 units across 19 sessions) in the foveal representation of the primary visual cortex (V1) in two awake marmosets. Subjects were trained to maintain stable fixation within a 2° window while viewing a repeated sequence of rapidly flashed, whitened natural images presented at 20 Hz (Fig. 1A). Eye position was monitored with a high-precision digital dual Purkinje image eye tracker \cite{wu2023high, ressmeyer2025openirisdpi, yates2023detailed} and typically remained well within the fixation window across trials (Fig. 1B). The distributions of eye position during fixation were similar to those observed in humans and macaques \cite{cherici2012precision, ressmeyer2025openirisdpi}. Receptive fields (RFs) of recorded units were confined to eccentricities of $<$1° (Fig. 1C). As expected, activity was elicited in the recorded units by the visual stimuli presented within their receptive fields (RF), and the resulting patterns of spikes displayed variability between trials (Fig. 1D).

Trial-to-trial variability was strongly related to the fine-scale position of the eye. For each unit we measured the displacement of the eye between trials, projected onto the axis across which the unit's receptive field is tuned (Fig. 1E, extended data figure 1). Sorting the stimulus-evoked responses by this displacement revealed clear structure in the trial-to-trial fluctuations (Fig. 1F), and splitting trials by the sign of the projection produced markedly different response timecourses (Fig. 1G). FEMs can therefore substantially structure the activity of individual units.
...
\begin{figure}[!t]
  ...
  \textbf{(F)} Per-pair noise correlation ($r_{\mathrm{sc}}$), uncorrected vs.\ FEM-corrected. Grey violins show the pooled per-pair distribution; the marker is the across-session mean $\pm$ SD and the grey band the shuffle null.
  \textbf{(G)} Participation ratio of the three covariance components --- residual ($\Sigma_{\text{res}}$), stimulus-locked ($\Cpsth$), and eye-movement--dependent ($\Cfem$). One faint line per session joins its three values; the marker is the across-session mean $\pm$ SD (log scale). Variance in the stimulus and FEM components is concentrated in comparably few dominant dimensions relative to the residual.
  \textbf{(H)} Schematic of the subspace-alignment test. FEM covariance (green disk) is projected orthogonally onto the subspace spanned by the leading eigenvectors of $\Cpsth$ (columns of $U$; blue plane). The fraction of variance retained, $f_{\mathrm{FEM}\to\mathrm{PSTH}} = \mathrm{tr}(U^\top\Cfem U)/\mathrm{tr}(\Cfem)$, measures how much FEM-driven variance lies within these dominant stimulus-locked dimensions. This is one of the two directions shown in (I), and is distinct from the FEM fraction of conditional rate variance in (C).
  \textbf{(I)} Variance captured in each direction --- stimulus-locked variance lying in the FEM subspace, and FEM variance lying in the stimulus-locked subspace --- with one point per session and the across-session mean $\pm$ SD. The grey dashed band is the shuffle null of the mean (the eye-trajectory shuffle). Both directions lie well above the null, indicating substantial overlap between the dominant dimensions of FEM-driven and stimulus-locked population variability. [All panels use a 25\,ms counting window. Significance markers ($\ast$) denote comparison against the eye-trajectory shuffle null (E, F, I) or paired tests across components (G); numeric values are reported in the main text.]}
\end{figure}

To quantify the contribution of FEMs to neural variability, we extended a recently developed method for variability decomposition (\cite{mcfarland2016variability}; Methods). Because the same time-varying image was presented on every trial, any two trials share the same stimulus at every given timepoint and differ only in the eye's position during the preceding moments. The similarity of their recent eye trajectories therefore determines whether the two spike counts arose from the same retinal input (Fig.~2A). We therefore estimated variability from only those trial pairs whose recent trajectories fell within a given distance of one another, and varied that distance (Fig.~2B). When the distance is large enough to admit every pair, the estimate recovers the eye-position-blind residual, which is the variability that remains once the trial-averaged response is subtracted. As the distance shrinks, only pairs whose eye positions nearly coincided over the preceding moments contribute, and the estimate falls toward a floor. The gap between the uncorrected residual and that floor is the variability driven by fixational eye movements, and the floor is the residual variability that survives once eye position is accounted for.

We define the conditional rate variance ($\sigma^2_{\text{rate}}$) as the total variance accounted for by matching trials at the same timepoint on their recent eye trajectories. It combines the variance of the trial-averaged response (i.e., the peri-stimulus time histogram [PSTH]) ($\sigma^2_{\text{PSTH}}$) and the trial-to-trial variance attributable to differences in the eye's position ($\sigma^2_{\text{FEM}}$). 
From these, we computed the fraction of the conditional rate variance attributable to fixational eye movements, which had a median of $\FigTwoFemMedian$ across the population (IQR $\FigTwoFemIQR$; $n=\FigTwoFemUnits$ units; Fig.~2C). Neural responses therefore varied more with trial-to-trial differences in eye position than with the stimulus itself, despite tight behavioral control.

Conventional measures of neural variability compare each trial to the trial-averaged response, so effects of differences in retinal input are assigned within the conventional residual ($\sigma^{2}_{\text{res},\mathrm{unc}} = \sigma^2_{\text{FEM}} + \sigma^2_{\text{res}}$). Our decomposition instead estimates the residual after accounting for differences in recent eye position between trials. This partitioning lowered estimates of the population Fano factor from $\FigTwoFanoUncorrected$ to a sub-Poisson value of $\FigTwoFanoCorrected$ (a reduction absent under a trajectory-shuffle null, empirical $p \leq 0.001$; Fig.~2E). Units in foveal V1 are therefore substantially less variable than the conventional residual implies.

The decomposition also partitions covariance across simultaneously recorded units into stimulus-locked ($\Cpsth$), eye-movement--dependent ($\Cfem$), and residual ($\Sigma_{\mathrm{res}}$) components (Methods; Fig.~2D). Correlations computed from the conventional residual, which combines the latter two components ($\Sigma_{\mathrm{res,unc}} = \Cfem + \Sigma_{\mathrm{res}}$), are conventionally termed ``noise correlations'' ($r_{\mathrm{sc}}$) \cite{cohen2011measuring,kohn2005stimulus}. The off-diagonal structure of this conventional residual was almost entirely attributable to $\Cfem$, leaving the FEM-corrected residual ($\Sigma_{\mathrm{res}}$) nearly diagonal. Before FEM correction, noise correlations averaged $\FigTwoNCUncorrected$ (95\% CI $\FigTwoNCUncorrectedCI$), comparable to previous reports \cite{smith2008spatial, mitchell2009spatial}. After correction, they fell to $\FigTwoNCCorrected$ (95\% CI $\FigTwoNCCorrectedCI$), with a confidence interval that included zero. This reduction exceeded that expected under the trajectory-shuffle null ($\Delta r_{\mathrm{sc}} = \FigTwoNCDifference$, 95\% CI $\FigTwoNCDifferenceCI$; shuffle-null interval $\FigTwoNCNullCI$; empirical $p \leq 0.001$; Fig.~2F). Differences in eye position across trials thus account for almost all of the shared variability left by the trial-averaged response.

FEM-driven variability was also concentrated in a few dominant population dimensions. The participation ratio of $\Cfem$ averaged $\FigTwoPRFem$ across sessions, roughly seven-fold more compact than the residual ($\FigTwoPRResidual$) and not detectably different from the stimulus-locked covariance, $\Cpsth$ ($\FigTwoPRStimulus$; sign test, $p = \FigTwoPRSignP$; Fig.~2G). With much of the variance in both components concentrated in a few dimensions, those dimensions could overlap or be largely distinct. Both arrangements are plausible. Changes in the displayed image and eye movements both alter the visual features falling within neuronal receptive fields, potentially recruiting similar population response patterns and aligning the dominant dimensions. However, a neuron's responsiveness across images need not predict its sensitivity to displacements of the stimulus. Small translations may strongly modulate neurons whose receptive-field subregions lie near local image contours while leaving other visually responsive neurons relatively unaffected. FEM-driven variability could therefore occupy dimensions distinct from those dominating the stimulus-locked response. We therefore asked whether the dominant population dimensions of these two covariance components aligned.

We quantified this alignment by computing the fraction of each component's variance captured by the four leading eigenvectors of the other (Fig.~2H; Methods). Larger fractions indicate greater overlap with the other component's dominant population dimensions. Because this measure is asymmetric, we evaluated both directions, each against a trajectory-shuffle null that broke the coupling between eye position and population activity. The leading FEM subspace captured a majority of the stimulus-locked variance ($\FigTwoStimInFem$), and the leading stimulus-locked subspace captured a majority of the FEM-driven variance as well ($\FigTwoFemInStim$; means $\pm$ SD across sessions). Both fractions exceeded their respective shuffle nulls ($\approx 0.35$; $p \leq 0.001$ for both directions; Fig.~2I). FEM-driven variability therefore largely overlaps the dominant subspace spanned by the stimulus-locked population response. 


%\clearpage

\subsection{Retinal input predicts FEM-dependent population activity}
...
\begin{figure}[t]
  ...
  \textbf{(B)} Model architecture and input interventions. A gaze-contingent retinal movie spanning approximately $0.9^\circ \times 0.9^\circ \times 250\,\mathrm{ms}$ ($35\times35$ pixels $\times$ 60 lags) passes through 14 learned spatiotemporal filters and three spatial convolutional stages. Features from all three stages are combined into a multiscale representation. A separate pathway transforms eye position and velocity into multiplicative gains on visual features and 64 additional feature channels appended before the readout. Each unit's readout combines spatial and feature weights, followed by a softplus nonlinearity to predict firing rate. The model shares 3.47 million parameters across 2,790 recorded units from 30 sessions, with 655 readout parameters fitted separately for each unit. The three conditions in C--E use identical fitted weights. Full (blue) retains both inputs; Retinal only (red) zeroes the extraretinal input; Stabilized (ochre) fixes the retinal crop at one session-global gaze position while retaining extraretinal input and stimulus changes.
    ...
    \textbf{(E)} Distribution of the FEM fraction of conditional rate variance ($\sigma^2_{\mathrm{FEM}}/\sigma^2_{\mathrm{rate}}$) for the recorded units and each model condition. Downward triangles mark distribution medians. The empirical median was $\FigThreeEEmpirical$, compared with $\FigThreeEFull$ for the full model, $\FigThreeERetinal$ for the retinal-only model, and $\FigThreeEStabilized$ for the stabilized-retina model. The shaded band shows the empirical median $\pm0.1$. Paired two-one-sided tests with this equivalence margin supported equivalence for the full and retinal-only models ($p=\FigThreeEFullTostP$ and $p=\FigThreeERetinalTostP$, respectively), but not for the stabilized-retina model ($p=\FigThreeEStabilizedTostP$). The empirical and stabilized-retina estimates differed by a median of $\FigThreeEStabilizedDifference$ (paired Wilcoxon $p=\FigThreeEStabilizedP$). In C and D, boxes span the interquartile range, median lines mark the medians, and whiskers span the 10th--90th percentiles across units.
  }
  \label{fig:model}
\end{figure}


The prior analyses show that modulation associated with FEMs overlaps the dominant dimensions of the stimulus-related response, but they do not identify the source of that modulation. Under one account, fixational eye movements displace the image on the retina and influence activity in V1 through primarily feed-forward circuits originating in the photoreceptors (i.e., reafference). Under the other, an internal signal reporting the eye's position and motion modulates V1 independently of the image on the retina (i.e., extraretinal modulation). Both accounts are consistent with the reported population geometry, but they assign the variability to fundamentally different sources.

We dissociated these two potential accounts statistically by fitting a single image-computable feedforward neural network model to the recorded V1 population. The model was trained to predict responses to gratings, Gabors, and natural images during free viewing \cite{yates2023detailed}. The fixated flashed-image condition analyzed throughout this paper was withheld from training, so the Figure~3 predictive metrics measure generalization across both stimulus and behavioral context (Fig.~3A).

The model predicts neural responses from a 250-ms retinal history using a full-history spatiotemporal convolution followed by three spatial convolutional stages and unit-specific readouts (Fig.~3B). The retinal input is gaze-contingent, so fixational eye movements move the image across the model's input window. A separate extraretinal pathway converts eye position and temporally filtered eye velocity into feature gains and spatially constant additive inputs using a multilayer perceptron model architecture. This is the only route by which eye state can affect the model's response without changing the retinal input. (``Extraretinal'' here refers to this pathway alone; the model does not identify whether an analogous biological signal arises from an efference copy, proprioception, or some other indirect source). Holding the fitted weights fixed, we compared the full model against two ablated versions. In the retinal-only condition, we set the extraretinal input to zero, testing whether that pathway improves prediction once retinal image motion has been specified. In the complementary stabilized-retina condition, we fixed the retinal sampling position for every trial at the center of each session's gaze distribution, removing the image motion caused by fixational eye movements while the extraretinal input remained intact.

The full and retinal-only models achieved similar trial-averaged accuracy. The full model predicted trial-averaged responses with a median noise-normalized correlation of $\mathrm{CC}_{\mathrm{norm}} = \FigThreeCFull$ across $n = \FigThreeCUnits$ units from \FigThreeSessions{} sessions (Fig.~3C). Removing the extraretinal input to create a retinal-only model yielded a small performance decrease (median $\FigThreeCRetinal$; median paired $\Delta \mathrm{CC}_{\mathrm{norm}} = \FigThreeCRetinalDelta$; paired signed-rank $p = \FigThreeCRetinalP$). Conversely, stabilizing the retinal input had a larger detrimental effect (median $\FigThreeCStabilized$; median paired $\Delta = \FigThreeCStabilizedDelta$; $p = \FigThreeCStabilizedP$). The paired losses were \FigThreeCRetinalReduction\% and \FigThreeCStabilizedReduction\% of the full-model median, respectively. Much of the trial-averaged response remained predictable, motivating a more stringent assessment at the level of single trials.
...
We therefore scored each model condition's prediction on single trials by the fraction of the conditional rate variance measured in Figure~2 that it captured (Methods). This score is zero for a constant predictor and one for an ideal predictor of the conditional rate. For comparison, we predicted each trial's response from the average of the remaining repeats. This leave-one-out trial average estimates the stimulus-locked response that defines the prediction ceiling when trial-to-trial fluctuations are treated as noise \cite{schoppe2016measuring}. It captured a median of $\FigThreeDPsth$ of the rate variance, whereas the full model captured $\FigThreeDFull$ (median paired $\Delta = +\FigThreeDFullGain$, $95\%$ CI $\FigThreeDFullGainCI$; $n = \FigThreeDUnits$ units from \FigThreeSessions{} sessions; session-clustered bootstrap, Methods; Fig.~3D). This amounted to a \FigThreeDGainPercent\% improvement over the trial-average median, suggesting that the model captured substantial FEM-driven variability around the mean response.


Most of the model's single-trial predictive advantage persisted when the explicit extraretinal inputs were removed. Zeroing those eye position and velocity terms caused a small loss of captured variance (median $\FigThreeDRetinal$; median paired $\Delta = \FigThreeDRetinalLoss$, $95\%$ CI $\FigThreeDRetinalLossCI$). In contrast, stabilizing the retinal input reduced the median fraction to $\FigThreeDStabilized$, below the trial-average median (paired $\Delta = \FigThreeDStabilizedLoss$, $95\%$ CI $\FigThreeDStabilizedLossCI$). This paired loss was \FigThreeDLossPercent\% of the full-model median. Single-trial prediction therefore depended strongly on gaze-contingent retinal input; removing the separate eye-state input cost substantially less than globally stabilizing the retinal image.

In an additional analysis, we asked over what timescale reafferent input contributed to accurate model predictions. We compared fixing gaze at one position for the entire session, at each trial's mean position, or at the latest position within each 250-ms input history. Preserving each history's final retinal position avoided most of the loss in accuracy caused by fixing gaze across whole trials or sessions. Thus, accurate prediction depended more on tracking moment-to-moment retinal position than on retaining the motion leading to that position (\ref{fig:stabilization_control}).

We then asked whether the model reproduced the relative magnitude of FEM-driven modulation measured directly from the recorded units. We applied the same decomposition used in Figure 2 to each condition's predicted rates and compared the resulting FEM fractions with those estimated from the recorded responses (Methods). The full and retinal-only models produced similar FEM fractions, with medians of $\FigThreeEFull$ and $\FigThreeERetinal$, respectively, compared with an empirical median of $\FigThreeEEmpirical$. Paired equivalence tests supported mean empirical-minus-model differences within a $\pm0.1$ margin (TOST $p = \FigThreeEFullTostP$ and $p = \FigThreeERetinalTostP$, respectively; Fig.~3E). In contrast, stabilizing the retinal input reduced the median FEM fraction to $\FigThreeEStabilized$ (median paired empirical-minus-model difference $\FigThreeEStabilizedDifference$; paired signed-rank $p = \FigThreeEStabilizedP$). Within the fitted model, retinal input alone was sufficient to reproduce the observed FEM fraction, and global stabilization reduced its median by more than half.

These results provide statistical evidence for a substantial reafferent contribution to FEM-dependent activity in foveal V1. Although the model need not reflect the underlying biological circuitry by construction, it does demonstrate that retinal input alone is sufficient to capture the single-trial predictive advantage over the trial average and to reproduce the observed FEM fraction. The small additional predictive contribution of the extraretinal pathway is consistent with extraretinal influences playing a secondary, modulatory role under the conditions of our experiments.

%\clearpage

\subsection{Fixational eye movements sharpen spatial population codes}

The preceding analyses showed large FEM-dependent changes in foveal V1 activity and provided evidence for a substantial reafferent contribution. Once gaze was accounted for, little shared variability remained. This raises a question: if shared variability is driven by FEMs, what does the retinal motion itself do to the visual representation? This cannot be measured directly in the recorded neurons for two reasons. First, the recorded dataset provides no way to observe the same neural population under the counterfactual condition in which retinal motion is removed or otherwise manipulated. Second, no experimental technique can record from complete populations of cortical neurons, so any population-level account necessarily requires inference about neurons we did not record from. We therefore used the fitted encoding model both to construct complete spatial populations and to interrogate counterfactual eye movements directly.
...
\textbf{A}, A filtered, measured eye-movement trace and its stabilized counterpart generate 250-ms natural-image movies matched at the mean first-layer peak lag. Horizontal arrows indicate that each movie passes through the same trained model to generate a spatial activation map for one illustrative unit. Both response maps show response divided by its spatial mean on a shared color scale; the rightmost map shows motion minus stabilized on a symmetric scale. The complete spatial maps are displayed without cropping. Motion increases firing rate and single-spike information (SSI) and sharpens the normalized spatial response.
\textbf{B}, Firing-rate and SSI changes versus fixation path length across \FigFourUnits{} units. Colored curves show spike-weighted population estimates.
\textbf{C}, Filled contours of equal-mass natural-image power for \FigFourDriftWindows{} drift windows and \FigFourMicrosaccadeWindows{} microsaccade-containing windows. Both conditions use one-second records filtered continuously with a positive Gaussian kernel ($\sigma=6$ ms); microsaccade amplitudes are below $1^\circ$. Each trace's nonzero-temporal-frequency power is normalized to unit mass before averaging with equal animal weights. Blue and orange contours show fitted \FigFourPassbandPercent\%-response passbands for two examples selected from the \FigFourValidated{} strictly validated units; the same contours appear in \textbf{D}.
\textbf{D}, Log$_2$ microsaccade/drift power ratio with the example passbands overlaid, showing their overlap with the redistributed retinal power.
\textbf{E}, Passbands for all \FigFourUnits{} model units, including \FigFourUnvalidated{} that do not pass the stricter validation criteria. Grayscale shading shows population passband occupancy.
\textbf{F}, Firing-rate and SSI modulation versus the within-unit percentile of motion-induced passband power. Greater passband engagement is associated with increases in both quantities. Colored curves show population medians.
\textbf{G}, Motion-dependent modulation across the cumulative model readout for high-passband-engagement movies. Gain-invariant temporal modulation (left) and SSI sharpening (right) increase as successive feature stages contribute to the output.
In \textbf{B}, \textbf{F}, and \textbf{G}, gray boxes show across-unit interquartile ranges and medians; whiskers show the 5th--95th percentiles across units. Colored points and capped bars show population estimates and 95\% bootstrap confidence intervals. Stars indicate positive effects relative to zero improvement ($p<0.05$, two-sided bootstrap tests with Holm correction across all 32 displayed effects; Methods).
}

\end{figure}


We therefore measured the joint spatial- and temporal-frequency tuning of the \textcolor{red}{\textbf{matched}} model units using controlled drifting gratings (Methods). These units occupied a broad range of SF$\times$TF passbands---the combinations of spatial and temporal frequencies that strongly drive each unit (Fig.~4E). he two example contours in Fig.~4C provide contrasting spatiotemporal passbands against which to compare the retinal spectra. \textcolor{red}{\textbf{Relative to drift windows}}, microsaccade-containing windows redistributed normalized natural-image power toward higher temporal frequencies, increasing the power fraction within one example passband while decreasing it within the other (Fig.~4D). \textcolor{red}{\textbf{Thus, the same change in fixation dynamics alters the spectral overlap with different model-derived passbands.}}

We next asked whether this spatiotemporal engagement predicted the response to individual fixation histories. For each of the \FigFourUnits{} model units, we calculated the motion-induced power entering its joint SF$\times$TF passband across fixation trajectories. Both firing-rate and SSI modulation increased with passband engagement (Fig.~4F): the median unit effect rose from \FigFourRateEngagementLow\% in the lowest engagement quintile to \FigFourRateEngagementHigh\% in the highest for firing rate, and from \FigFourSSIEngagementLow\% to \FigFourSSIEngagementHigh\% for SSI. Within-unit Spearman correlations were larger for passband engagement than for path length (rate: median $\rho = \FigFourRatePassbandRho$ versus $\FigFourRatePathRho$, paired difference $\FigFourRateRhoDifference$, 95\% CI $\FigFourRateRhoCI$; SSI: $\FigFourSSIPassbandRho$ versus $\FigFourSSIPathRho$, paired difference $\FigFourSSIRhoDifference$, 95\% CI $\FigFourSSIRhoCI$; unit bootstrap). These descriptive correlations motivated a comparison that jointly accounted for \textcolor{red}{\textbf{movement distance and class}}.

With the \textcolor{red}{\textbf{image ensemble}} fixed, \textcolor{red}{\textbf{adding engagement}} to a model containing movement class and class-specific nonlinear path-length effects reduced held-out squared prediction error by a median \FigFourCompareRateReduction\% for rate (95\% CI \FigFourCompareRateCI\%) and \FigFourCompareSSIReduction\% for SSI (\FigFourCompareSSICI\%; source-trial bootstrap). To distinguish spectral distribution from the amount of retinal modulation, we divided each movie's passband power by its total dynamic power and tested this normalized overlap with total power also included in the baseline. The additional reduction was \FigFourNormalizedSSIReduction\% for SSI (95\% CI \FigFourNormalizedSSICI\%) and \FigFourNormalizedRateReduction\% for rate (\FigFourNormalizedRateCI\%). The SSI benefit persisted in the strictly validated tuning subset (\FigFourNormalizedStrictSSIReduction\%, 95\% CI \FigFourNormalizedStrictSSICI\%). Thus, the estimated spectral distribution contributed modest predictive information about spatial sharpening beyond movement distance, movement class, and overall dynamic power in this image ensemble.

The spectral calculation identifies which motion-generated signals engage a neuron's spatiotemporal tuning, but it does not explain how those signals become a sharper spatial representation. Focusing only on spectral power discards Fourier phase, whereas the model operates on the complete spatiotemporal movie. We therefore replayed matched moving and stabilized movies through the fitted model and accumulated the trained readout contributions from successive spatial stages (Fig.~4G). Spatial sharpening was small for the first-stage contribution and larger after the second and third stages were included. These cumulative readouts reconstruct the full model output at the final stage; \textcolor{red}{\textbf{they describe how the trained model combines features, rather than isolating causal effects of individual stages.}}

\section{Discussion}

Neural activity supporting central vision is continuously shaped by self-generated motion of the eyes, even when gaze is tightly regulated in a task that requires no sampling movement. Measuring this required tracking gaze at the scale of foveal receptive fields \reviewedit{\cite{wu2023high, ressmeyer2025openirisdpi, yates2023detailed}} while recording dense populations of neurons. Decomposing the population covariance showed that fixational eye movements were the dominant source of rate variability in the foveal population, contributing more than the repeated sequence of natural images we presented.

Conventional assays of neuronal noise (i.e., residual variability) combine internally-generated fluctuations with effects of differences in retinal input that are not captured by the trial-averaged response \cite{tolhurst1983statistical, shadlen1998variable}. In the fovea of alert primates, that treatment misattributes a large fraction of the residual variability, because gaze follows a different trajectory on every trial. Accounting for those trajectories lowered the population Fano factor from $\FigTwoFanoUncorrected$ to a sub-Poisson $\FigTwoFanoCorrected$ and reduced the mean pairwise correlation from $\FigTwoNCUncorrected$ to $\FigTwoNCCorrected$, consistent with the empirical summaries above. Fixational eye movements accounted for nearly all of the shared variability that the trial-averaged response did not explain.

Measured spike-count correlations span two orders of magnitude across studies, and much of that spread has been attributed to features of the preparation rather than of the circuit, including anesthetic state \cite{ecker2014state}, slow fluctuations in gain \cite{goris2014partitioning}, firing rate \cite{delarocha2007correlation}, and the counting window and spike-sorting criteria \cite{cohen2011measuring}. Recurrent excitatory--inhibitory circuits can decorrelate neural populations \cite{renart2010asynchronous}, but correlations measured during fixation can also reflect shared changes in retinal input. Our results show that trial-to-trial differences in eye position can inflate estimates of noise correlation in experiments with awake animals. Fortunately, eye position differs from other factors by being readily and directly monitored non-invasively with instrumentation that is now widely available \cite{wu2023high, ressmeyer2025openirisdpi, yates2023detailed}, and its influence can be accounted for with the decomposition presented here.

FEM-driven variability overlapped the dominant dimensions of the stimulus-locked population response, showing that even small eye movements substantially reshape activity within these dimensions across stimulus presentations. Primates nevertheless resolve fine spatial detail during fixation, with psychophysical evidence showing that retinal motion supports spatial resolution \cite{rucci2007miniature, intoy2020finely}. Taken together, these observations motivate a single-trial account of population coding and readout in V1. Variability around the trial average includes systematic responses to image features moving across receptive fields, even when the displayed scene remains unchanged. These responses are part of the visually driven signal, although conventional analyses often assign them to noise.

How the brain uses signals generated by eye movements depends on the task at hand. Extraretinal eye-position signals can contribute to judgments about where objects lie in external space \cite{bridgeman1991ocular}. However, studies that decouple retinal motion from ongoing eye movements show that fine spatial discrimination can benefit from that motion even without correspondence to the eye's trajectory \cite{ratnam2017benefits}. Such judgments could use displacement inferred from visual activity or spatial relationships preserved across retinal translations. Which mechanisms support each task remains a question for future work. Here, we examined how the gaze-dependent responses arise by comparing the predictive contributions of retinal input and explicit eye-state signals.

We compared the predictive contributions of retinal and explicit eye-state inputs using an image-computable model that predicted individual trials better than the cross-trial average. Removing explicit eye-position and eye-velocity signals left single-trial prediction and eye-driven modulation largely intact, whereas stabilizing the retinal input substantially reduced both. Preserving current retinal position while removing motion within each input history recovered much of the global-stabilization loss. This  is consistent with the flashed-image paradigm used to drive neurons: the $3^\circ$ images changed at 20~Hz, providing temporal modulation even with fixed gaze, with eye position determining which features fell within the small foveal receptive fields (or the subfields of simple-type receptive fields). Recent retinal motion made an additional contribution to single-trial prediction, although the impact of removing it was not demonstrably larger than that of the explicit extraretinal input. This supports a substantial reafferent contribution: much of the measured modulation can be predicted from changes in the visual input itself, as suggested by earlier physiological work. Kagan and colleagues showed that retinal image motion and position account for distinct V1 responses to saccades and drifts, alongside extraretinal modulation in a sample of \textcolor{red}{\textbf{22}} analyzable neurons \cite{snodderly2001selective,kagan2008saccades}. Our population analysis extends the basic notion of eye movements impacting receptive field drive by quantifying how strongly retinal input predicts single-trial responses and FEM-dependent rate modulation \textcolor{red}{\textbf{and covariance}}.

This evidence for reafference links the variability associated with FEMs to a well-established function of retinal motion. Fixational motion converts spatial structure into temporal modulation, redistributing natural-image power into the temporally band-pass regime of early visual neurons \cite{rucci2005fixational,kuang2012temporal}, and retinal stabilization impairs fine spatial vision \cite{rucci2007miniature,intoy2020finely}. Our results connect this input transformation to the cortical population code. Measured fixation trajectories produced both stronger responses and greater spatial information per predicted spike than when the model was stimulated with perfectly stable inputs. \textcolor{red}{\textbf{Passband engagement added predictive information about both effects beyond movement distance and microsaccade classification. After controlling total dynamic power, normalized spectral overlap retained a modest predictive benefit for SSI. This supports a contribution of the retinal signal's spectral distribution to model spatial sharpening beyond these movement descriptors and the overall amount of modulation. It does not establish unique neuron--passband routing or exclude other descriptions of eye-movement dynamics.}}

The spectral redistribution describes the signal generated by FEMs, but not the full computation that produces a sharper spatial representation. The passband analysis retains power while discarding Fourier phase, whereas the fitted model operates on the complete spatiotemporal movie. Adding successive feature-stage contributions to the trained readout increased both temporal modulation and spatial information for \textcolor{red}{\textbf{high-engagement movies}}. This analysis characterizes the \textcolor{red}{\textbf{cumulative readout and does not isolate a causal contribution of each internal stage}}. In the fitted model, FEMs make spatial structure available as a temporally modulated signal, and subsequent transformations produce a more spatially informative response. The specific spatiotemporal tuning and stagewise transformations identified here are model-derived predictions that can be tested directly with controlled dynamic stimuli.

A downstream circuit could amplify or attenuate these changes depending on which V1 responses it pools. Attention models make the relevant distinction explicit: modulation aligned with the gradient of task performance can be more effective than modulation based on neuronal tuning alone \cite{lindsay2018attention}. Analogously, FEM-dependent changes could have larger effects downstream when they engage inputs to which a later computation is sensitive. This suggests a testable connection to attention: changes in fixation dynamics may alter the retinal signals available to downstream selection. Because experiments did not manipulate attention or measure downstream responses, they do not explain effects previously attributed to attention, but they do motivate joint consideration of these factors.

Active vision is often considered at the scale of saccades, with each fixation treated as a static sample \cite{ballard1991animate,najemnik2005optimal,butko2010infomax,yang2016theoretical}. Our results instead show that active sensing continues throughout fixation. Movements of fractions of a degree account for much of the apparent variability in foveal V1 while simultaneously generating signals that improve spatial encoding. Our results show that accounting for retinal image motion changes how cortical variability is interpreted and reveals a visual signal that can sharpen spatial representations.
...
Metrics were computed for units with mean firing rates above 2 spikes/s and split-half PSTH reliability $R^2 > 0.10$ \cite{schoppe2016measuring}. We defined $R^2$ as the squared Pearson correlation between split-half PSTHs, averaged across 100 random trial splits. Sessions with fewer than 10 qualifying units were excluded. These criteria retained a mean of $79.6 \pm 26.1$ units per session for monkey A and $18.2 \pm 9.1$ for monkey L (mean $\pm$ s.d. across sessions). Primary results used a 25-ms counting window; analyses at 8.3, 16.7, and 50 ms assessed robustness to counting-window duration.
...
\begin{equation}
r_{\mathrm{sc},cd} = \frac{\Sigma_{cd}}{\sqrt{\Sigma_{cc}\Sigma_{dd}}}.
\end{equation}
...
\reviewedit{We additionally tested history-local stabilization (\ref{fig:stabilization_control}). For each native 240-Hz prediction, we rendered all 60 input frames at the ROI of that prediction's latest input sample, retaining each historical frame's actual flashed image and display position. Thus the current retinal frame is identical to the measured input, while preceding gaze displacement is removed. This is a history-specific counterfactual, not one globally consistent stabilized movie: retinal position can change from one prediction to the next. Both stabilization controls retain the measured behavioral inputs and the temporal modulation from the 20-Hz image sequence. Rendering was checked against the native renderer and every current frame was verified pixel-for-pixel. We used the same canonical observations, validity masks, neuron identities, correlation ceilings, affine calibration procedure, and conditional-rate-variance denominators as Figure~3C,D. Full and retinal-only inputs were replayed as numerical reproducibility controls; the additional analysis did not recompute the FEM variance decomposition. To compare ablation costs, we subtracted each ablated score from Full within each unit. The difference between the recent-motion and extraretinal costs is therefore the Retinal-only score minus the History-local score; we summarized this paired contrast directly, rather than subtracting population medians. Paired effects were summarized with 10,000 session bootstrap resamples, conditional on the fitted model.}
...
\begin{equation}
\frac{\sigma^2_{\mathrm{FEM}}}{\sigma^2_{\mathrm{rate}}}
=
...
\subsubsection{Passband engagement compared with movement descriptors}

\reviewedit{We tested whether engagement improved prediction of rate and SSI modulation beyond movement class and path length using the same 40-image $\times$ 200-history response matrix. The histories comprised 146 drift and 54 microsaccade windows from 187 source trials, with 100 histories from each animal. The primary analysis retained the image reductions used for Figure~4F. Separate regressions for each of the 725 units predicted rate and SSI percent modulation from movement class, path length, engagement, or combinations of these predictors. All models included animal identity, and evaluation weighted the animals equally. The joint class--path baseline allowed a different nonlinear path-length relationship in each movement class.}

\reviewedit{Each continuous predictor was transformed to its empirical percentile using training observations only, then represented by five cubic B-spline terms with interior knots at one-third and two-thirds of the percentile range. Tied training values used their midrank; test values outside the training range saturated at zero or one. Features were centered and standardized on training data. Ridge regression used a fixed penalty of 0.01 relative to weighted mean squared loss, with 0.001 and 0.1 as sensitivity checks. Five-fold cross-validation grouped all histories from one source trial, stratified by animal and movement class, and was repeated with three fixed random seeds. Squared test errors were averaged across repetitions; predictions were not averaged before scoring.}

\reviewedit{The principal contrast was the paired reduction in held-out mean squared error after adding engagement to the class--path baseline, summarized by the median across the fixed model units. Specificity controls added total dynamic movie power to both models, substituted population-average engagement, or substituted another neuron's engagement using 32 fixed random assignments with no self-matches. Each shuffled assignment was held constant across folds and outcomes; its prediction errors were averaged over assignments. Effects were also summarized within each movement class, within each animal, and in the 145-unit strict tuning-validation subset. Conventional partial rank correlations controlled for ranked path length, movement class, and animal identity.}

\reviewedit{A follow-up audit isolated spectral distribution using normalized overlap
\[
q_u = \frac{\sum_{s,t,o} P(s,t,o)W_u(s,t,o)}{\sum_{s,t,o}P(s,t,o)},
\]
where $P$ is the movie's dynamic power tensor and $W_u$ is the unit-mass passband tensor. This weighted overlap is invariant to a scalar rescaling of a movie's power. We verified that stabilized spectral power was numerically zero and that all dynamic-power denominators were positive, normalized each movie separately, and then took the median over the 40 images. We compared the class--path--total-power baseline against that baseline plus normalized overlap, using the same folds, training-only spline transformations, penalty, and paired scoring. This is a simple additive test of the estimated spectral distribution; it does not exhaust nonlinear effects of retinal structure.}

\reviewedit{A secondary analysis of raw engagement retained individual movies and crossed five source-canvas folds with five source-eye-trial folds. Training excluded both the test canvas group and the test eye-trial group; every movie was predicted once. The 40 patches came from 13 distinct source canvases. For each neuron and history, centering targets and out-of-fold predictions over scenes additionally isolated variation across images at fixed movement. Confidence intervals used 1,000 paired bootstrap resamples of source eye trials within animal for the primary analysis, and crossed source-canvas and source-eye-trial resamples for individual movies. Neurons shared the same resampling weights and were not independently resampled. These intervals condition on the fitted twin and regression models; primary intervals also condition on the fixed image ensemble. They do not estimate neural-noise, model-training, or between-animal uncertainty. In this secondary analysis, adding raw engagement to the class--path baseline reduced prediction error by \FigFourCompareMovieRateReduction\% for rate (95\% CI \FigFourCompareMovieRateCI\%) and \FigFourCompareMovieSSIReduction\% for SSI (\FigFourCompareMovieSSICI\%); both intervals included zero. The fixed-ensemble comparisons therefore do not establish generalization of these descriptors across scenes.}

\reviewedit{The specificity controls are conditional on the spectral estimator. With total dynamic power included in the baseline, raw engagement gave additional error reductions of \FigFourComparePowerRateReduction\% for rate (95\% CI \FigFourComparePowerRateCI\%) and \FigFourComparePowerSSIReduction\% for SSI (\FigFourComparePowerSSICI\%), and own-passband assignments did not outperform shuffled assignments. The 250-ms movie window gives a 4-Hz Fourier-bin spacing; the $NW=1.5$ tapers have a nominal 6-Hz half-bandwidth. In a noise-free calibration using the production estimator, normalized power estimates for known 1-Hz and 2-Hz complex temporal carriers had cosine similarity \FigFourCarrierResolutionCosine{}. Across units, image-reduced engagement predictors had median pairwise rank correlation \FigFourEngagementPredictorRho{}. These diagnostics demonstrate limited spectral resolution and highly similar predictors, but do not establish the cause of the shuffle result. Failure to outperform a shuffled passband therefore cannot exclude tuning-specific effects that this finite-window descriptor does not resolve.}

\subsubsection{Cumulative network-stage analysis}

To trace the high-passband effect through the trained model, Figure~4G used fixation histories selected solely from the top 20\% of each unit's passband-power distribution. \FigFourStageTraces{} histories spanning the high-engagement range were crossed with \FigFourStageImages{} natural images, producing \FigFourStageMovies{} measured/stabilized movie pairs. Unit effects were summarized from the selected histories lying within that unit's top quintile.
...
\end{equation}
Figure~4G reports $100(M_{\mathrm{motion}}-M_{\mathrm{stabilized}})$ in percentage points. Spatial sharpening was the expected-spike-weighted absolute difference
...
\reviewedit{For the zero-improvement tests in Figure~4B,F,G, we retained each panel's population estimand and paired bootstrap scheme described above. Two-sided bootstrap sign-tail probabilities were computed as $p=\min\{1,2[1+\min(n_{\leq0},n_{\geq0})]/(N_{\mathrm{boot}}+1)\}$, where $n_{\leq0}$ and $n_{\geq0}$ count resampled population effects on either side of zero. We applied Holm correction jointly across all 32 bin- or stage-specific tests (two outcomes in eight path-length bins, five engagement quintiles, and three cumulative readouts). These tests assess the population summaries under the stated resampling schemes, conditional on the fitted model; the boxplots separately describe heterogeneity across units.}
...
\begin{figure}[p]
  \centering
  \includegraphics[width=\linewidth]{figures/stabilization_control.pdf}
  \caption{\textbf{Prediction accuracy declines as retinal stabilization extends from input histories to whole trials and sessions.}
  All conditions use the Figure~3 model with fixed weights and the same scored trials and units. Full retains the measured retinal and extraretinal inputs. Retinal, corresponding to the retinal-only condition in Figure~3, retains the measured retinal history and removes the explicit extraretinal input. Global, corresponding to the stabilized-retina condition in Figure~3, fixes gaze at one session-wide reference. Trial stabilization fixes gaze near each trial's mean position. History-local stabilization fixes gaze at the latest input position separately within each 250-ms history, preserving the final retinal frame while removing preceding gaze displacement. All stabilization conditions retain the explicit extraretinal inputs and the actual 20-Hz flashed-image sequence.
  \textbf{(A)} Trial-averaged noise-normalized correlation ($n=\StabCUnits$ units). Medians were \StabCFull{}, \StabCRetinal{}, \StabCGlobal{}, \StabCTrial{}, and \StabCHistory{} for Full, Retinal, Global, Trial, and History-local, respectively. Median paired changes were \StabCFullToHistory{} for History-local minus Full (95\% CI \StabCFullToHistoryCI{}), \StabCHistoryToTrial{} for Trial minus History-local (\StabCHistoryToTrialCI{}), and \StabCTrialToGlobal{} for Global minus Trial (\StabCTrialToGlobalCI{}).
  \textbf{(B)} Fraction of conditional rate variance captured on single trials ($n=\StabDUnits$ units), using the Figure~3D denominator. Medians for the same five conditions were \StabDFull{}, \StabDRetinal{}, \StabDGlobal{}, \StabDTrial{}, and \StabDHistory{}. Median paired changes were \StabDFullToHistory{} for History-local minus Full (95\% CI \StabDFullToHistoryCI{}), \StabDHistoryToTrial{} for Trial minus History-local (\StabDHistoryToTrialCI{}), and \StabDTrialToGlobal{} for Global minus Trial (\StabDTrialToGlobalCI{}). Removing the extraretinal input reduced prediction by \StabDExtraLoss{} (\StabDExtraLossCI{}). The within-unit difference between recent-motion and extraretinal ablation costs was \StabDCostDifference{} (\StabDCostDifferenceCI{}), which did not establish a larger cost for removing recent motion. The dashed line marks the PSTH median (\StabDPsth{}); the solid line marks zero captured variance.
  Preserving each history's final retinal position avoided most of the loss caused by stabilization across whole trials or sessions, with a smaller contribution from motion within the history. It is worth noting that the 20-Hz flashed-image sequence provided sustained temporal drive even when gaze was held fixed. Recent retinal motion is therefore expected to contribute more under naturalistic conditions without rapid image flashes, as explored in the model analyses of Figure~4.
  Boxes span the interquartile range, with median lines and 10th--90th percentile whiskers. The 95\% confidence intervals reported in the caption were calculated using 10,000 paired session-bootstrap samples across 19 sessions.}
  \label{fig:stabilization_control}
\end{figure}
\clearpage
