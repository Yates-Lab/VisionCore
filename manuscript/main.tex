...
Global stabilization changes both the current retinal position and its recent history. To distinguish these contributions, we froze gaze separately within each 250-ms history at its current input position (\ref{fig:stabilization_control}). This history-local control preserved the current retinal frame and recovered much of the predictive loss: median trial-averaged correlation was $\StabCHistory$ and single-trial captured rate variance was $\StabDHistory$. Relative to the full model, the paired change in trial-averaged correlation was $\StabCFullDelta$ (95\% CI $\StabCFullCI$). Removing recent retinal motion still reduced single-trial prediction, with a median paired loss of $\StabDMotionLoss$ (95\% CI $\StabDMotionLossCI$), compared with $\StabDExtraLoss$ ($\StabDExtraLossCI$) for removing the explicit extraretinal input. The direct within-unit difference between these costs was $\StabDCostDifference$ ($\StabDCostDifferenceCI$; $n=\StabDUnits$ units), which did not establish a larger contribution of recent motion. Thus, most of the global-stabilization loss was avoided when current retinal position was retained, with a further predictive contribution from motion within the input history.

We then asked whether the model reproduced the FEM modulation measured directly from the recorded units, by applying the decomposition approach to each condition's predicted rates (Figure~2; Methods). The full and retinal-only models accounted for similar fractions of the rate modulation, with medians of $\FigThreeEFull$ and $\FigThreeERetinal$, compared with an empirical median of $\FigThreeEEmpirical$. Paired equivalence tests supported agreement within a $\pm0.1$ margin (TOST $p = \FigThreeEFullTostP$ and $p = \FigThreeERetinalTostP$, respectively; Fig.~3E). Stabilizing the retinal input shifted the model median to $\FigThreeEStabilized$, outside that margin (median paired empirical-minus-model difference $\FigThreeEStabilizedDifference$; paired signed-rank $p = \FigThreeEStabilizedP$). Within the fitted model, retaining retinal image motion was sufficient to reproduce the magnitude of FEM-dependent rate modulation, and global stabilization reduced it by more than half.

These results provide statistical evidence for a substantial reafferent contribution to FEM-dependent activity in foveal V1. Within the fitted model, retinal image motion retained the single-trial explanatory advantage when the separate eye-state input was removed. The weaker contribution of that pathway does not exclude extraretinal modulation that is redundant with retinal input, incompletely measured, or inadequately represented by this architecture.
...
\begin{figure}[p]
  ...
  \caption{\textbf{Predictive effects of global and history-local retinal stabilization.}
  All conditions use the selected Figure~3 model with fixed weights and the same scored trials and units. Global stabilization holds gaze at one session-wide reference; history-local stabilization holds gaze at the current input position separately for each prediction's 250-ms history. Both retain the explicit extraretinal inputs and preserve the actual 20-Hz flashed-image sequence. The history-local control preserves the current retinal frame and allows its position to vary across predictions. Retinal-only retains the measured retinal history and removes the explicit extraretinal input.
  \textbf{(A)} Trial-averaged normalized correlation ($n=\StabCUnits$ units). Medians are \StabCFull{} for Full, \StabCRetinal{} for Retinal-only, \StabCGlobal{} for Global, and \StabCHistory{} for History-local. The paired History-local-minus-Full change is \StabCFullDelta{} (95\% CI \StabCFullCI{}).
  \textbf{(B)} Fraction of conditional rate variance captured on single trials ($n=\StabDUnits$ units), using the Figure~3D denominator. Corresponding medians are \StabDFull{}, \StabDRetinal{}, \StabDGlobal{}, and \StabDHistory{}; the dashed line marks the PSTH median. The median paired loss relative to Full is \StabDMotionLoss{} (95\% CI \StabDMotionLossCI{}) for History-local and \StabDExtraLoss{} (\StabDExtraLossCI{}) for Retinal-only. Their within-unit cost difference, equal to the Retinal-only score minus the History-local score, is \StabDCostDifference{} (\StabDCostDifferenceCI{}).
  Boxes span the interquartile range, with medians and 10th--90th percentile whiskers. Intervals use 10,000 paired session bootstrap samples across 19 sessions.}
