...
def audit_ablation_payload(
    ...
    *,
    ...
    for record in payload["results"]:
        ...
        # Older intact Figure-3 caches did not persist the data-only stability
        # mask explicitly, even though it is represented exactly by the NaNs
        # in their stored CCnorm vector.  The ablation builder already supports
        # that schema (see ``_renormalize_ccnorm_to_intact_anchor``); mirror the
        # same fail-closed derivation here so the independent audit can check
        # newly generated ablations against either intact-cache schema.
        anchor_unstable = np.asarray(
            anchor.get("ccnorm_unstable", ~np.isfinite(anchor["ccnorm"])),
            dtype=bool,
        )
        unstable_exact = _exact(record["ccnorm_unstable"], anchor_unstable)
