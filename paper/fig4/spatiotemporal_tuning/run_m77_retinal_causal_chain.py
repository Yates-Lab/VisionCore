#!/usr/bin/env python3
"""Score the renderer-faithful M77 retinal-motion causal chain.

This command consumes an independently selected natural-image table, the
native-240 real-fixation bank, and the dense zero-behavior RR100 tuning table.
For every requested image/trace/motion-scale condition it computes the exact
retinal movie spectrum, projects that spectrum onto measured RR100 tuning, and
plays the identical movie through M77 to obtain rate, expected spikes, and
...
EPS = 1e-12
MODEL_OUTPUT_RATE_HZ = 240.0
DEFAULT_BASE = ROOT / "outputs/dekel240_paper/m77_epoch279"
DEFAULT_MATRIX = DEFAULT_BASE / "figure4_real_trace_production_corrected/merged"
DEFAULT_CHAIN = DEFAULT_BASE / "retinal_causal_chain"
...
DEFAULT_DATASET = ROOT / "paper/model_selection/configs/multi_240_long_split3_dekel35.yaml"
DEFAULT_POPULATION = ROOT / "outputs/redundancy_resolved_v1_twin/step1_activation_fingerprints"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--tuning-table",
        type=Path,
        default=DEFAULT_CHAIN / "dense_tuning/frequency_tuning_grouped.csv",
    )
    ...
    parser.add_argument(
        "--trace-transform",
        choices=("identity", "rotate90"),
        default="identity",
        help="Rotate each complete trajectory without changing its path length.",
    )
...
def tuning_tensors(table: pd.DataFrame) -> dict[str, np.ndarray]:
    """Return complete signed-rate and phase-RMS RR100 tuning tensors."""
    required = {
        "unit_index",
        "spatial_cpd",
        "temporal_hz",
    ...
    amplitude = dynamic.response_amp_rms.to_numpy(dtype=np.float64).reshape(shape)
    ...
    signed = MODEL_OUTPUT_RATE_HZ * (mean_rate - static[:, :, None, :])
    passband = MODEL_OUTPUT_RATE_HZ * np.clip(amplitude, 0.0, None)
...
def load_tuning_tensors(path: Path) -> dict[str, np.ndarray]:
    """Load a validated tensor cache, building it once from the grouped CSV."""
    source = Path(path)
    cache = source if source.suffix == ".npz" else source.parent / "rr100_tuning_tensors.npz"
    if cache.exists() and (
    ...
    result = tuning_tensors(pd.read_csv(source))
...
def score_retinal_movie(
    ...
    *,
    ...
    return {
        "mean_rate": MODEL_OUTPUT_RATE_HZ * rate.mean(axis=0),
...
def main() -> int:
    args = parse_args()
    if args.history_samples != 60 or args.analysis_samples != 240:
        raise ValueError("production M77 contract requires 60 history + 240 analysis samples")
    ...
    if args.trace_transform == "rotate90":
        selected_traces = np.stack(
            (-selected_traces[..., 1], selected_traces[..., 0]), axis=-1
        )
    ...
    if scorer.input_rate_hz != 240 or scorer.output_rate_hz != 240 or scorer.n_lags != 60:
        raise RuntimeError("scorer is not the native-240 M77 contract")
            ...
            for scale_index, scale in enumerate(scales):
                ...
                if temporal_hz is None:
                    ...
                    signed_controls = load_signed_projection_controls(
                        args.tuning_table.parent / "rr100_signed_projection_controls_240hz.npz",
                        signed,
                        frequency,
    ...
    summary = {
        "analysis": "renderer-faithful zero-behavior M77 retinal causal-chain shard",
        ...
        "rate_units": "spikes/s; native Poisson outputs multiplied by 240 Hz",
        "expected_spike_definition": "sum of native expected spike counts across the exact one-second analysis interval",
