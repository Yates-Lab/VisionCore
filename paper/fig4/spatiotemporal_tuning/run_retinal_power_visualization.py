#!/usr/bin/env python3
"""Build auxiliary-aperture retinal SF×TF×orientation power maps.
...
scorer aperture in ``run_retinal_causal_chain.py``.  Every spectrum here
...
def evenly_spaced_rows(length: int, count: int) -> np.ndarray:
    """Return deterministic, unique rows spanning a source table."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--frame-rate-hz", type=float, default=240.0)
...
def render_power_figure(
    ...
    *,
    ...
    figure.suptitle(
        f"Direct-rendered {aperture_size_px}-pixel retinal movies · complete real trajectories",
        fontsize=15,
        fontweight="semibold",
    )
...
def main() -> int:
    ...
    figure = args.out_dir / "auxiliary_retinal_power.png"
    render_power_figure(
        figure,
        scales=scales,
    ...
    summary = {
        ...
        "aperture_size_px": int(args.aperture_size_px),
        "frame_rate_hz": float(args.frame_rate_hz),
        "analysis_seconds": float(args.analysis_samples / args.frame_rate_hz),
