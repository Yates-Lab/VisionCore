...
The legacy summary fits a single correlated log-Gaussian to phase-RMS
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--grouped-csv", type=Path, required=True)
    parser.add_argument("--out-dir", type=Path, required=True)
    parser.add_argument(
        "--model-spec",
        type=Path,
        default=ROOT / "paper/model_selection/selected_manuscript_model.yaml",
        help=(
            "Selected-model manifest used to reject tuning from another checkpoint. "
            "Ignored when --expected-checkpoint is supplied."
        ),
    )
    parser.add_argument(
        "--expected-checkpoint",
        ...
        default=None,
        help=(
            "Explicit checkpoint whose SHA-256 must match the periodic-tuning "
            "provenance. Use this for model-agnostic audits without changing the "
            "selected manuscript-model manifest."
        ),
    )
    parser.add_argument(
        "--model-label",
        default="",
        help="Optional display label; otherwise read from periodic-tuning provenance.",
    )
...
def source_provenance(
    ...
    provenance_path = grouped_csv.parent / "periodic_tuning_provenance.json"
    if not provenance_path.exists():
        raise FileNotFoundError(
            "a production tuning audit requires periodic_tuning_provenance.json "
            f"beside the grouped responses: {provenance_path}"
        )
    provenance = json.loads(provenance_path.read_text(encoding="utf-8"))
    if expected_checkpoint is None:
        selected = yaml.safe_load(model_spec.read_text(encoding="utf-8"))
        expected = str(selected.get("checkpoint_sha256", ""))
        expected_source = str(model_spec.resolve())
    else:
        expected_checkpoint = expected_checkpoint.resolve()
        if not expected_checkpoint.is_file():
            raise FileNotFoundError(
                f"expected checkpoint does not exist: {expected_checkpoint}"
            )
        selected = {}
        expected = sha256(expected_checkpoint)
        expected_source = str(expected_checkpoint)
    observed = str(
        provenance.get("checkpoint_sha256")
        or provenance.get("model_info", {}).get("checkpoint_sha256", "")
    )
    if not observed or not expected or observed != expected:
        raise ValueError(
            "periodic-tuning responses do not match the expected model checkpoint: "
            f"observed={observed!r}, expected={expected!r}, "
            f"expected_source={expected_source!r}"
        )
    if not bool(provenance.get("complete_grid", False)):
        raise ValueError("periodic-tuning response grid is incomplete")
    if int(provenance.get("contract", {}).get("output_rate_hz", 0)) != 240:
        raise ValueError("periodic-tuning responses are not from the native 240-Hz model")
    return {
        "path": str(provenance_path.resolve()),
        "sha256": sha256(provenance_path),
        "model_label": str(provenance.get("model_label", selected.get("label", "model"))),
        "checkpoint_sha256": observed,
        "expected_checkpoint_source": expected_source,
        "dataset_config_sha256": str(provenance.get("dataset_config_sha256", "")),
        "grid": provenance.get("grid", {}),
        "complete_grid": True,
        "n_population_units_scored": int(provenance.get("n_population_units_scored", 0)),
    }
...
def analyze(table: pd.DataFrame) -> tuple[pd.DataFrame, dict[int, dict]]:
    ...
    for coordinate, suffix in (("sf", "cpd"), ("tf", "hz")):
        rms = summary[f"rms_preferred_{coordinate}_{suffix}"].to_numpy(dtype=float)
        f1 = summary[f"f1_preferred_{coordinate}_{suffix}"].to_numpy(dtype=float)
        consensus = np.sqrt(rms * f1)
        consensus[~(np.isfinite(consensus) & (consensus > 0))] = np.nan
        summary[f"consensus_preferred_{coordinate}_{suffix}"] = consensus
    summary["validated_tuning"] = summary.audit_category.eq("trusted")
    summary["validated_preferred_sf_cpd"] = summary.consensus_preferred_sf_cpd.where(
        summary.validated_tuning
    )
    summary["validated_preferred_tf_hz"] = summary.consensus_preferred_tf_hz.where(
        summary.validated_tuning
    )
...
def population_figure(summary: pd.DataFrame, out_dir: Path, model_label: str) -> Path:
    configure()
    figure, axes = plt.subplots(2, 3, figsize=(15.2, 7.8), constrained_layout=True)
    trusted = summary.loc[summary.validated_tuning].sort_values(
        "validated_preferred_sf_cpd"
    )
    if trusted.empty:
        raise ValueError("no trusted unit is available for the population audit figure")
    exemplar = trusted.iloc[len(trusted) // 2]
    exemplar_label = f"u{int(exemplar.unit_index):03d}"
    ...
    figure.suptitle(
        f"{model_label} joint SF/TF tuning audit: local interpolated maxima must follow the raw peak",
        fontsize=13,
        fontweight="semibold",
    )
    path = out_dir / "tuning_fit_audit_population.png"
...
def example_gallery(
    ...
    figure.suptitle(
        f"{model_label} tuning-fit audit: raw samples, fitted shape, and estimator sensitivity",
        fontsize=13,
        fontweight="semibold",
    )
    figure.tight_layout(rect=(0, 0, 1, 0.975), h_pad=1.5, w_pad=1.0)
    path = out_dir / f"tuning_fit_audit_examples_page{page}.png"
...
def main() -> int:
    ...
    grouped_csv = args.grouped_csv.resolve()
    provenance = source_provenance(
        grouped_csv,
        args.model_spec.resolve(),
        expected_checkpoint=args.expected_checkpoint,
    )
    model_label = str(args.model_label or provenance["model_label"])
    table = pd.read_csv(grouped_csv)
    summary, details = analyze(table)
    summary_path = args.out_dir / "tuning_fit_audit.csv"
    summary.to_csv(summary_path, index=False)
    population_path = population_figure(summary, args.out_dir, model_label)
    trusted = summary.loc[summary.validated_tuning].sort_values(
        "validated_preferred_sf_cpd"
    )
    if len(trusted) < 4:
        raise ValueError("fewer than four trusted tuning estimates are available")
    trusted_rows = np.unique(
        np.rint(np.linspace(0, len(trusted) - 1, 4)).astype(int)
    )
    trusted_units = trusted.iloc[trusted_rows].unit_index.astype(int).tolist()
    questionable = summary.loc[~summary.validated_tuning].sort_values(
        ["audit_category", "unit_index"]
    )
    questionable_units = questionable.head(4).unit_index.astype(int).tolist()
    galleries = [
        example_gallery(summary, details, trusted_units, args.out_dir, 1, model_label),
        example_gallery(summary, details, questionable_units, args.out_dir, 2, model_label),
    ]
    counts = summary.audit_category.value_counts().reindex(CATEGORY_COLORS, fill_value=0)
    exemplar = trusted.iloc[len(trusted) // 2]
    validated = summary.loc[summary.validated_tuning].copy()
    required_validated_columns = [
        "validated_preferred_sf_cpd",
        "validated_preferred_tf_hz",
    ]
    finite_validated = bool(
        np.isfinite(validated[required_validated_columns].to_numpy(dtype=float)).all()
        and (validated[required_validated_columns].to_numpy(dtype=float) > 0).all()
    )
    population_checks = {
        "at_least_24_trusted_units": int(len(validated)) >= 24,
        "validated_coordinates_are_finite_and_positive": finite_validated,
        "median_rms_f1_sf_disagreement_at_most_quarter_octave": float(
            summary.rms_vs_f1_sf_difference_octaves.median()
        ) <= 0.25,
        "median_rms_f1_tf_disagreement_at_most_quarter_octave": float(
            summary.rms_vs_f1_tf_difference_octaves.median()
        ) <= 0.25,
        "at_least_80_percent_positive_heldout_shape_r2_rms": float(
            summary.rms_heldout_r2.gt(0).mean()
        ) >= 0.80,
        "at_least_80_percent_positive_heldout_shape_r2_f1": float(
            summary.f1_heldout_r2.gt(0).mean()
        ) >= 0.80,
    }
    report = {
        "analysis": f"{model_label} continuous SF/TF peak audit",
        "model_label": model_label,
        "checkpoint_sha256": provenance["checkpoint_sha256"],
        "expected_checkpoint_source": provenance["expected_checkpoint_source"],
        "n_units": int(len(summary)),
        "n_validated_units": int(len(validated)),
        "response_metrics": {
        ...
        "trusted_tuning_contract": {
            "source_response": "native-240-Hz periodic drifting gratings on the complete recommended SF x TF x orientation grid",
            "coordinate_estimator": "geometric mean of independently estimated phase-RMS and F1 local joint-quadratic peaks",
            "quality_gate": "audit_category == trusted",
            "sf_column": "validated_preferred_sf_cpd",
            "tf_column": "validated_preferred_tf_hz",
            "unit_indices": validated.unit_index.astype(int).tolist(),
            "minimum_units_for_sf_terciles": 24,
        },
        "release_checks": population_checks,
        "release_ready": bool(all(population_checks.values())),
        "classification": {
        ...
        "source": {
            ...
            "grouped_csv_sha256": sha256(grouped_csv),
            "periodic_tuning_provenance": provenance,
            "model_spec": str(args.model_spec.resolve()),
        },
        "table": str(summary_path.resolve()),
    ...
    print(json.dumps({"audit_exemplar": report["audit_exemplar"]}, indent=2))
    print(json.dumps({"release_ready": report["release_ready"]}, indent=2))
    return 0
