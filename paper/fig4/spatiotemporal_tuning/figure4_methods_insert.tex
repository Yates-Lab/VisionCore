...
zero-phase positive Gaussian kernel ($\sigma=6$ ms, truncated at five standard
deviations), and sampled at the model's native 240-Hz
...
Filtering used 0.6 s of continuous padding on each side. The Gaussian's gentle
rolloff near the independently measured DDPI spectral breakpoint avoids the
overshoot of the earlier sharp elliptic filter. Widths of 4 and 8 ms were
evaluated as retinal-power spectral sensitivity controls. Windows lacking sufficient raw padding,
valid native tracking, or reliable event coverage were excluded from the
event comparison. Saved event times and sample bounds were checked against
the current DDPI record, accounting for session affine calibration differences;
amplitudes were measured from current DDPI displacement at those sample bounds.
...
The two conditions were event-free drift windows and equal-duration windows
containing verified microsaccades with $0<\mathrm{amplitude}<1^\circ$.
Events had to lie fully inside the scored window with 50-ms margins; drift
windows had no event within 50 ms of either edge. Larger events, unmatched
rapid movements, and uncertain coverage were excluded. Selection did not use
spectral centroids, neural activity, or model responses.
