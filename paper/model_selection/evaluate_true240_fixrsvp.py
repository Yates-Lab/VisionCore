#!/usr/bin/env python3
"""Evaluate a truly 240-Hz twin on FixRSVP and on Figure 3's 120-Hz frame.

The selected model predicts one 4.17-ms spike-count bin at a time.  For the
Figure-3 comparison, adjacent native predictions and observations are summed
into the same non-overlapping 8.33-ms bins used by the published 120-Hz twin.
...
from evaluate_dekel_fixrsvp import (  # noqa: E402
    CCNORM_N_SPLITS,
    FIXATION_RADIUS_DEG,
    MIN_FIX_DUR,
    VALID_TIME_BINS,
...
def _representative_examples(results, references, n_examples=4):
    ...
    for result, reference in zip(results, references):
        ...
        ryan = np.asarray(reference["ccabs"], dtype=float)
        stable = np.isfinite(result["ccnorm"]) & np.isfinite(reference["ccnorm"])
        for local in np.flatnonzero(stable & np.isfinite(candidate) & np.isfinite(ryan)):
            pool.append(
                {
                    "result": result,
                    "reference": reference,
                    ...
                    "delta": float(candidate[local] - ryan[local]),
                }
            )
    if not pool:
    ...
    specifications = [
        ...
        ("Twin advantage", "delta", float(np.quantile(delta_values, 0.10))),
    ]
    chosen = []
    used = set()
...
def _plot_examples(
    ...
    output_path,
    *,
    candidate_label="native-240 candidate",
    n_examples=4,
):
    ...
    for row, (selection_label, result, reference, unit) in enumerate(chosen):
        ...
        ryan_valid = _data_score_mask(reference["robs_used"], reference["dfs_used"])
        ryan_valid &= np.isfinite(reference["rhat_used"])
        ryan_pred = np.nanmean(np.where(ryan_valid, reference["rhat_used"], np.nan), axis=0)
        t120 = np.arange(m76_data.shape[0]) / 120.0 * 1000.0
        ax = axes[row, 1]
        ax.plot(t120, m76_data[:, unit] * 120, color=colors["data"], lw=1.35, label="data")
        ax.plot(t120, ryan_pred[:, unit] * 120, color=colors["ryan"], lw=1.35, label="Twin")
        ax.plot(
            t120,
            m76_pred[:, unit] * 120,
            color=colors["candidate"],
        ...
        ax.text(
            ...
            f"Twin {reference['ccnorm'][unit]:.2f}\n"
...
def main():
    ...
    parser.add_argument(
        "--candidate-label",
        default="native-240 candidate",
        help="Human-readable model name used in plots and provenance",
    )
    parser.add_argument(
        "--figure3-cache",
        type=Path,
    ...
    reference_by = _reference_by_session(args.figure3_cache)
