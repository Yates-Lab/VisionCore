...
def _representative_examples(results, references, n_examples=4):
    ...
    specifications = [
        ...
        ("Ryan advantage", "delta", float(np.quantile(delta_values, 0.10))),
    ]
    chosen = []
    used = set()
...
def _plot_examples(
    ...
    *,
    ...
    for row, (selection_label, result, reference, unit) in enumerate(chosen):
        ...
        ax.plot(t120, ryan_pred[:, unit] * 120, color=colors["ryan"], lw=1.35, label="Ryan")
        ax.plot(
            t120,
            m76_pred[:, unit] * 120,
            color=colors["candidate"],
        ...
        ax.text(
            ...
            f"Ryan {reference['ccnorm'][unit]:.2f}\n"
