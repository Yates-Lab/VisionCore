"""Shared, fail-closed protocol for fixed-RSVP model evaluation.

The helpers reproduce Figure 3's trial frame, cell identities, positive affine
Poisson calibration, and normalized-correlation estimator. They are separate
from the command-line evaluator so protocol behavior can be unit tested
without loading a checkpoint or dataset.
"""
...
def _prepare_fixrsvp(config):
    ...
    # A descriptive grating-calibration checkpoint may record that every
    # grating repeat was exposed to training.  FixRSVP was never a training
    # stimulus, and this helper deliberately replaces the bank list, so the
    # grating-only override must not leak into data preparation here.
    config.pop("train_all_stimulus_types", None)
...
def _session_metrics(arrays, neuron_mask, reference=None):
    ...
    if reference is not None:
        ...
        # the new model prediction changes.
...
def _audit_shared_ccnorm(current, reference):
    """Rescore a reference model and a candidate on identical support and noise ceilings."""
    ...
    if not np.array_equal(current["ccnorm_support"], reference_cc["support"]):
        raise AssertionError("Candidate and reference model do not use identical CCnorm support")
    if not np.allclose(
        current["ccmax"], reference_cc["ccmax"], rtol=0, atol=1e-12, equal_nan=True
    ):
        raise AssertionError("Candidate and reference model CCmax differ on identical data")
    if not np.array_equal(current["ccnorm_unstable"], reference_cc["unstable"]):
        raise AssertionError(
            "Candidate and reference model CCnorm stability masks differ on identical data"
        )
