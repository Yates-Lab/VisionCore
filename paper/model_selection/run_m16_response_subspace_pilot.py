#!/usr/bin/env python3
"""Response-subspace pilot for the 240-Hz M16 digital twin.

This is an isolated M16 analogue of the Figure-4 response-subspace pilot.  It
uses the same frozen RR100 population definition, but evaluates each unit at
the centre of M16's 35-pixel crop instead of assuming the older twin's
translation-equivariant 151 -> 51 spatial lattice.
"""
...
DEFAULT_DATASET_CONFIG = ROOT / (
    "paper/model_selection/configs/multi_240in_120target_long_split3_dekel35.yaml"
)
...
DEFAULT_OUT = ROOT / "outputs/figures/m16_reproductions/fig4_response_subspace"
MCFARLAND_OUTPUTS = ROOT / "scripts/mcfarland_outputs_mono.pkl"
RYAN_SUBSPACE_SUMMARY = ROOT / (
...
def generate_movie_batch(
    ...
    *,
    ...
    """Fixed-covariance Gaussian contexts on M16's normalized input scale."""
...
def build_m16_rr100_readout(
    teacher: M16Teacher,
    population_view,
    canonical_rows: list[dict],
    rr_unit_indices: np.ndarray,
) -> DirectPopulationReadout:
    """Map requested frozen RR100 units onto M16 by session and biological CID."""
    ...
    if missing:
        raise RuntimeError(
            "Selected RR100 units are absent from M16's stored CID subset: "
            + json.dumps(missing)
        )
...
def load_teacher(args: argparse.Namespace, selection: pd.DataFrame):
    ...
    teacher = M16Teacher(model=model, device=args.device)
    readout = build_m16_rr100_readout(
        teacher,
        population_view,
    ...
    with torch.no_grad():
        ...
        if tuple(core.shape[-3:]) != (1, SCAFFOLD_SIZE, SCAFFOLD_SIZE):
            raise RuntimeError(f"Unexpected M16 core shape {tuple(core.shape)}")
...
def render(
    ...
    if RYAN_SUBSPACE_SUMMARY.exists():
        ryan = pd.read_csv(RYAN_SUBSPACE_SUMMARY)
        ryan = ryan.loc[ryan["rank"].isin(ranks)]
        axes[0].plot(
            ryan["rank"], ryan["median_test_rate_r2"], "s--",
            color="#d06a2e", lw=1.8, label="Ryan median",
        )
...
def main() -> int:
    ...
    if args.robust_tuning_summary is not None:
        ...
        amplitudes = (
            ...
            .rename(columns={"response_amp_rms": "current_dynamic_peak_response_amp"})
        )
        unit_table = unit_table.merge(
            ...
            how="inner",
            validate="one_to_one",
        ).merge(amplitudes, on="unit_index", how="inner", validate="one_to_one")
