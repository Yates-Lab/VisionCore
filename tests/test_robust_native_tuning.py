...
import pandas as pd
import torch
from paper.fig4.spatiotemporal_tuning.robust_native_tuning import _surface_prediction, fit_local_quadratic_peak, fit_log_gaussian_surface, recommended_native_grid
from paper.fig4.spatiotemporal_tuning.compute_native_rucci_overlap import native_kinematic_occupancy, overlap_table
from paper.fig4.spatiotemporal_tuning.analyze_image_specific_joint_engagement import circular_orientation_weights, image_unit_engagement, interpolate_tuning_to_fft_bins, log_interpolation_weights, trajectory_phase_spectra, validate_matrix_contract, validate_native240_trace_contract
from paper.fig4.spatiotemporal_tuning.analyze_direct_rendered_joint_engagement import engagement_from_cubes
...
def test_log_gaussian_fit_recovers_interior_continuous_peak() -> None:
    ...
    params = np.asarray([0.08, np.log(1.7), np.log2(target_sf), np.log2(target_tf), np.log(0.8), np.log(1.0), 0.2])
...
def test_boundary_peak_is_never_reported_as_uncensored() -> None:
    ...
    temporal = np.sqrt(2.0) ** np.arange(14)
    xx, yy = np.meshgrid(np.log2(spatial), np.log2(temporal))
    params = np.asarray([0.05, np.log(1.0), np.log2(spatial[-1]), np.log2(temporal[-1]), np.log(0.7), np.log(0.7), 0.0])
    response = _surface_prediction(params, xx, yy)
    fit = fit_log_gaussian_surface(spatial, temporal, response)
...
def test_local_quadratic_peak_recovers_joint_interior_maximum() -> None:
    ...
    response = 0.1 + np.exp(-0.5 * (np.square((xx - np.log2(target_sf)) / 0.72) + np.square((yy - np.log2(target_tf)) / 0.84) - 0.35 * (xx - np.log2(target_sf)) * (yy - np.log2(target_tf))))
...
def test_local_peak_tracks_observed_maximum_when_global_surface_has_a_shoulder() -> None:
    ...
    xx, yy = np.meshgrid(np.log2(spatial), np.log2(temporal))
    narrow = np.exp(-0.5 * (np.square((xx - np.log2(2.8)) / 0.55) + np.square((yy - np.log2(22.0)) / 0.52)))
    broad_shoulder = 0.72 * np.exp(-0.5 * (np.square((xx - np.log2(2.0)) / 1.2) + np.square((yy - np.log2(5.0)) / 2.0)))
...
def test_native_kinematic_occupancy_maps_k_dot_v_without_fft(tmp_path) -> None:
    ...
    occupancy, audit = native_kinematic_occupancy(mode_power=np.asarray([3.0]), kxy=np.asarray([[2.0, 0.0]]), traces=traces, frame_rate_hz=240.0, spatial_cpd=np.asarray([np.sqrt(2), 2.0, 2 * np.sqrt(2)]), temporal_hz=np.asarray([1.0, 2.0, 4.0, 8.0]), orientation_deg=np.asarray([0.0, 90.0]))
    assert occupancy[2, 1, 1, 1] > 0
    assert occupancy[3, 1, 2, 1] > 0
...
def test_rucci_overlap_uses_stable_weighted_center_sf_tertile_tails() -> None:
    ...
    robust = pd.DataFrame({'unit_index': units, 'weighted_center_sf_cpd': [3.0, 1.0, 6.0, 2.0, 5.0, 4.0], 'fit_r2': np.ones(6), 'peak_censoring': ['none'] * 6, 'low_sf_censored': [False] * 6, 'preferred_tf_hz': np.ones(6), 'low_tf_censored': [False] * 6, 'high_tf_censored': [False] * 6})
...
def test_joint_engagement_interpolates_log_frequency_and_orientation() -> None:
    lower, upper, low_weight, high_weight, valid = log_interpolation_weights(np.asarray([np.sqrt(2.0)]), np.asarray([1.0, 2.0, 4.0]))
    assert valid[0]
    assert (lower[0], upper[0]) == (0, 1)
    np.testing.assert_allclose([low_weight[0], high_weight[0]], [0.5, 0.5])
    o0, o1, w0, w1 = circular_orientation_weights(np.asarray([[0.0, 2.0]]), np.asarray([0.0, 45.0, 90.0, 135.0]))
...
def test_trajectory_phase_spectrum_recovers_constant_translation_frequency() -> None:
    ...
    trace[0, :, 0] = 4.0 * time
    temporal_hz, power = trajectory_phase_spectra(np.asarray([[2.0, 0.0]]), trace, rate_hz, chunk_size=1)
...
def test_trajectory_phase_spectrum_uses_temporal_order_not_velocity_histogram() -> None:
    ...
    _, blocked = trajectory_phase_spectra(np.asarray([[8.0, 0.0]]), traces[:1], rate_hz, chunk_size=1)
    _, alternating = trajectory_phase_spectra(np.asarray([[8.0, 0.0]]), traces[1:], rate_hz, chunk_size=1)
    assert not np.allclose(blocked, alternating, rtol=0.05, atol=1e-08)
...
def test_brownian_phase_spectrum_broadens_with_spatial_frequency() -> None:
    ...
    diffusion = 0.015
    increments = rng.normal(scale=np.sqrt(2.0 * diffusion / rate_hz), size=(n_traces, n_time, 2))
    traces = np.cumsum(increments, axis=1)
    temporal_hz, power = trajectory_phase_spectra(np.asarray([[1.0, 0.0], [8.0, 0.0]]), traces, rate_hz, chunk_size=2)
...
def test_joint_engagement_preserves_sf_tf_pairing_until_final_dot_product() -> None:
    ...
    tuning[0, 0, 0, 0] = 0.5
    tuning[0, 1, 1, 0] = 0.5
    contract = {'sf0': np.asarray([0, 1]), 'sf1': np.asarray([0, 1]), 'sw0': np.ones(2), 'sw1': np.zeros(2), 'ori0': np.zeros(2, dtype=int), 'ori1': np.zeros(2, dtype=int), 'ow0': np.ones(2), 'ow1': np.zeros(2), 'resolved': np.ones(2, dtype=bool), 'temporal': np.asarray([[1.0, 0.0], [0.0, 1.0]])}
...
def test_joint_alignment_fraction_removes_global_image_power_scale() -> None:
    ...
    np.testing.assert_allclose(engagement['joint_fraction'][0, 0], engagement['joint_fraction'][1, 0])
...
def test_direct_rendered_joint_score_preserves_pairing_and_separable_control() -> None:
    ...
    np.testing.assert_allclose(scores['separable_fraction'][0, 0], scores['separable_fraction'][1, 0])
...
def test_matrix_contract_validates_coordinates_shapes_and_integrity(tmp_path) -> None:
    ...
    pd.DataFrame({'image_index': range(n_images)}).to_csv(tmp_path / 'image_feature_table.csv', index=False)
    pd.DataFrame({'trace_index': range(n_traces)}).to_csv(tmp_path / 'trace_feature_table.csv', index=False)
    pd.DataFrame({'unit_index': range(n_units)}).to_csv(tmp_path / 'unit_feature_table.csv', index=False)
    ...
    for name in ('stabilized_ssi_by_image', 'stabilized_expected_spikes_by_image', 'stabilized_mean_rate_by_image'):
        np.save(tmp_path / f'{name}.npy', np.ones(stable_shape))
    np.save(tmp_path / 'trace_xy.npy', np.zeros((n_traces, 5, 2)))
    (tmp_path / 'summary.json').write_text(json.dumps({'n_images': n_images, 'n_traces': n_traces, 'n_units': n_units, 'integrity_checks': {'finite': True, 'nonnegative': True}}))
    (tmp_path / 'stabilized_baseline_summary.json').write_text('{}')
    audit = validate_matrix_contract(tmp_path)
    assert audit['declared_coordinates'] == {'n_images': n_images, 'n_traces': n_traces, 'n_units': n_units}
    assert all(audit['merge_integrity_checks'].values())

def test_native240_trace_contract_requires_true_scored_grid() -> None:
    contract = {'source_trace_rate_hz': 120, 'source_trace_samples': 40, 'model_output_rate_hz': 240, 'scored_samples_per_source_trace_sample': 2, 'scored_trace_samples': 80, 'scored_bin_seconds': 1.0 / 240.0, 'analysis_interval_seconds': 1.0 / 3.0}
...
def test_separable_control_preserves_marginals_but_removes_sf_tf_coupling() -> None:
    ...
    np.testing.assert_allclose(diagonal_scores['separable'], anti_diagonal_scores['separable'])
