...
def test_m77_production_config_builds_with_nonrecurrent_behavior():
    path = ROOT / (
        "experiments/model_configs/"
        "dekel_capacitymatched_freqmasked_readoutfloor0p5_"
        "groupnorm_lrnalpha0p1_mlp_behavior.yaml"
    )
    ...
    assert by_name["gaussian_width"] == ["readouts.0.std"]
    assert by_name["gaussian_width_floor"] == ["readouts.0.std"]
    assert by_name["behavior_mlp_weight"]
    assert config["convnet"]["params"]["frequency_mask"] == {
        "temporal": True,
        "stem_spatial": True,
    ...
    assert config["convnet"]["params"]["normalization"]["type"] == (
        "groupnorm_lrn_presplit"
    )
    assert model.recurrent.__class__ is torch.nn.Identity
    assert model.convnet.get_output_channels() == 504
...
@pytest.mark.skip(reason="sweep-only configs are retained on the archive branch")
