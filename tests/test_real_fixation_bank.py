...
from paper.fig4.spatiotemporal_tuning.build_real_fixation_bank import (
    ...
    spectral_filter_qc,
    trace_metrics,
    valid_saccade_events,
    within_window_event_summary,
)


def test_evenly_spaced_replay_subset_spans_complete_ordered_bank() -> None:
    indices = evenly_spaced_indices(1000, 200)

    assert len(indices) == 200
    assert indices[0] == 0
    assert indices[-1] == 999
    assert np.all(np.diff(indices) > 0)
    assert set(np.diff(indices)) <= {5, 6}
...
def test_guarded_fixations_retains_subdegree_events_and_guards_macro_events(tmp_path):
    path = tmp_path / "saccades.json"
    path.write_text(
        json.dumps(
            [
    ...
    trials = [{"trial_idx": 3, "trial_start_ephys": 0.0, "trial_stop_ephys": 2.5}]
    fixations = guarded_fixations(
        trials,
        path,
        guard_seconds=0.05,
        macro_saccade_min_amplitude_deg=1.0,
    )
    assert len(fixations) == 2
    assert np.isclose(fixations[0]["fixation_stop_ephys"], 1.45)
    assert np.isclose(fixations[1]["fixation_start_ephys"], 1.59)

    summary = within_window_event_summary(
        valid_saccade_events(path),
        scored_start=0.25,
        scored_stop=1.25,
        macro_saccade_min_amplitude_deg=1.0,
    )
    assert summary["saved_microsaccade_count"] == 1
    assert np.isclose(summary["saved_microsaccade_max_amplitude_deg"], 0.5)
...
def test_spectral_filter_qc_detects_stopband_suppression():
    time = np.arange(120) / 240.0
    raw_trace = np.sin(2 * np.pi * 5 * time) + 0.5 * np.sin(2 * np.pi * 48 * time)
    filtered_trace = np.sin(2 * np.pi * 5 * time)
    raw = np.broadcast_to(raw_trace[None, :, None], (4, 120, 2)).copy()
    filtered = np.broadcast_to(filtered_trace[None, :, None], (4, 120, 2)).copy()

    result = spectral_filter_qc(
        filtered,
        raw,
        history_samples=60,
    ...
    assert result["stopband_suppression_gate"]
    assert (
        result["filtered_power_fraction_above_threshold"]
        < result["raw_resampled_power_fraction_above_threshold"]
    )
