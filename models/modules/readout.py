...
__all__ = [
    'BaseFactorizedReadout', 'DynamicGaussianReadout',
    'ResidualGaussianReadout', 'FlattenedLinearReadout',
]
...
class ResidualGaussianReadout(nn.Module):
    """Zero-initialized second spatial component around a mature readout.

    The component owns a new neuron-specific channel projection, but expresses
    its spatial geometry relative to an existing ``DynamicGaussianReadout``.
    Its feature weights begin at exactly zero, so adding it to a checkpoint is
    an exact identity.  Once trained, its center can move a bounded distance
    and its width can broaden or narrow relative to the base component.  When
    the visual core is frozen, the resulting stimulus Jacobian is only a sum
    of derivatives already represented by that smooth feature map.
    """
    ...
    def __init__(
        ...
        max_base_scale=1.0,
        population_rank=4,
        max_population_mix=0.5,
    ):
        super().__init__()
        self.in_channels = int(in_channels)
        self.n_units = int(n_units)
        self.max_mean_delta = float(max_mean_delta)
        self.min_std_scale = float(min_std_scale)
        self.max_std_scale = float(max_std_scale)
        self.feature_mode = str(feature_mode)
        self.max_base_scale = float(max_base_scale)
        self.population_rank = int(population_rank)
        self.max_population_mix = float(max_population_mix)
        initial_std_scale = float(initial_std_scale)
        if self.in_channels <= 0 or self.n_units <= 0:
            raise ValueError("in_channels and n_units must be positive")
        if self.max_mean_delta < 0:
            raise ValueError("max_mean_delta must be non-negative")
        if self.feature_mode not in {
            "independent",
            "base_scaled",
            "base_population_lowrank",
        }:
            raise ValueError(
                "feature_mode must be 'independent', 'base_scaled', or "
                "'base_population_lowrank', got "
                f"{self.feature_mode!r}"
            )
        if self.max_base_scale <= 0:
            raise ValueError("max_base_scale must be positive")
        if self.population_rank <= 0:
            raise ValueError("population_rank must be positive")
        if self.max_population_mix <= 0:
            raise ValueError("max_population_mix must be positive")
        if not (
            0 < self.min_std_scale
            <= initial_std_scale
            <= self.max_std_scale
        ):
            raise ValueError(
                "Require 0 < min_std_scale <= initial_std_scale <= "
                "max_std_scale"
            )
        ...
        if self.feature_mode == "independent":
            ...
            self.register_parameter("population_left", None)
            self.register_parameter("population_right", None)
        elif self.feature_mode == "base_scaled":
            self.features = None
            # A bounded signed copy of the mature feature projection.  This is
            # a compact matched-surround model: it can be suppressive or
            # facilitatory without relearning hundreds of channel weights per
            # neuron.  Zero initialization preserves the source checkpoint
            # exactly.
            self.base_scale = nn.Parameter(torch.zeros(self.n_units))
            self.register_parameter("population_left", None)
            self.register_parameter("population_right", None)
        else:
            self.features = None
            self.register_parameter("base_scale", None)
            # A compact neuron-by-neuron mixing matrix factorized through a
            # small rank.  The right factor starts random while the left starts
            # at zero: the residual is therefore an exact identity at launch,
            # but gradients immediately select useful directions in the
            # population's already learned feature subspace.
            self.population_left = nn.Parameter(
                torch.zeros(self.n_units, self.population_rank)
            )
            self.population_right = nn.Parameter(
                torch.empty(self.n_units, self.population_rank)
            )
            nn.init.normal_(
                self.population_right,
                mean=0.0,
                std=1.0 / math.sqrt(self.population_rank),
            )
        self.mean_delta = nn.Parameter(torch.zeros(self.n_units, 2))
        self.log_std_scale = nn.Parameter(
            torch.full(
                (self.n_units, 2),
                math.log(initial_std_scale),
            )
        )
        self.theta_delta = nn.Parameter(torch.zeros(self.n_units))
        self._cached_grid = None
        self._cached_H = None
        self._cached_W = None

    def _create_grid(self, H, W, device, dtype):
    ...
    def effective_geometry(self, base_readout):
        """Return effective ``(mean, std, theta)`` for inspection/tests."""
        if not isinstance(base_readout, DynamicGaussianReadout):
            raise TypeError(
                "ResidualGaussianReadout requires a "
                "DynamicGaussianReadout base"
            )
        if base_readout.n_units != self.n_units:
            raise ValueError(
                "Residual/base readout unit mismatch: "
                f"{self.n_units} vs {base_readout.n_units}"
            )
        base_mean = base_readout.mean.detach()
        base_std = base_readout.std.detach().clamp(min=1e-3)
        base_theta = base_readout.theta.detach()
        mean = base_mean + self.max_mean_delta * torch.tanh(
            self.mean_delta
        )
        log_scale = self.log_std_scale.clamp(
            min=math.log(self.min_std_scale),
            max=math.log(self.max_std_scale),
        )
        std = base_std * torch.exp(log_scale)
        theta = base_theta + self.theta_delta
        return mean, std, theta

    def compute_gaussian_mask(self, H, W, device, dtype, base_readout):
        if H <= 0 or W <= 0:
            raise ValueError("Residual Gaussian spatial dimensions must be positive")
        if (
            H != self._cached_H
            or W != self._cached_W
            ...
            self._cached_grid = self._create_grid(H, W, device, dtype)
            self._cached_H = H
            self._cached_W = W

        mean, std, theta = self.effective_geometry(base_readout)
        mean = mean.to(device=device, dtype=dtype)
        std = std.to(device=device, dtype=dtype)
        theta = theta.to(device=device, dtype=dtype)
        centered_grid = self._cached_grid.unsqueeze(0) - mean[:, None, None]
        ...
        rotated_grid = torch.einsum(
            'nhwi,nij->nhwj', centered_grid, rotation
        )
        exponent = -0.5 * torch.sum(
            (rotated_grid / std[:, None, None]) ** 2,
            dim=-1,
        )
        mask = torch.exp(exponent)
        return mask / (mask.sum(dim=(-1, -2), keepdim=True) + 1e-8)

    def forward(self, x, base_readout):
        if x.dim() == 5:
            x = x[:, :, -1]
        if x.dim() != 4:
            raise ValueError(
                "ResidualGaussianReadout expects 4D or 5D input, got "
                f"{x.dim()}D"
            )
        ...
        if base_readout.in_channels < self.in_channels:
            raise ValueError(
                "Residual readout requests more channels than its base: "
                f"{self.in_channels} vs {base_readout.in_channels}"
            )
        if self.feature_mode == "independent":
            feature_map = self.features(x)
        elif self.feature_mode == "base_scaled":
            # Detaching makes the invariant explicit even if a future caller
            # forgets to freeze the source readout during residual-only fits.
            # The mature readout may also consume feature-space behavior
            # channels appended after the visual core.  The residual receives
            # only the declared leading visual slice, so reuse the matching
            # columns and never leak behavior into a stimulus surround.
            base_weight = base_readout.features.weight[
                :, :self.in_channels
            ].detach()
            feature_map = F.conv2d(x, base_weight)
            scale = self.max_base_scale * torch.tanh(self.base_scale)
            feature_map = feature_map * scale[None, :, None, None]
        else:
            base_weight = base_readout.features.weight[
                :, :self.in_channels
            ].detach()
            base_feature_map = F.conv2d(x, base_weight)
            mixing = self.population_left @ self.population_right.T
            mixing = self.max_population_mix * torch.tanh(
                mixing / math.sqrt(self.population_rank)
            )
            feature_map = torch.einsum(
                "ij,bjhw->bihw", mixing, base_feature_map
            )
        mask = self.compute_gaussian_mask(
            x.shape[-2],
            x.shape[-1],
            x.device,
            x.dtype,
        ...
        return (feature_map * mask.unsqueeze(0)).sum(dim=(-2, -1))
