...
class SignPairNormReLU(nn.Module):
    ...
    def forward_with_signed(
        self, x: torch.Tensor
    ) -> tuple[torch.Tensor, torch.Tensor]:
        """Return the ordinary split-ReLU output and its signed coordinate.

        Dekel's production ``groupnorm_lrn_presplit`` path normalizes a signed
        response before making positive/negative copies.  Exposing that signed
        response lets a neuron-localized shallow readout retain grating phase
        without changing the established multiscale scaffold.  For post-split
        normalization controls, the difference of the rectified pair is the
        corresponding signed coordinate.
        """
        if self.normalization == "groupnorm_lrn_presplit":
            signed = self.lrn(self.norm(x))
            return F.relu(_pairwise_sign_split(signed)), signed
        paired = _pairwise_sign_split(x)
        output = F.relu(self.lrn(self.norm(paired)))
        pairs = output.reshape(
            output.shape[0], output.shape[1] // 2, 2, *output.shape[2:]
        )
        signed = pairs[:, :, 0] - pairs[:, :, 1]
        return output, signed

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        return self.forward_with_signed(x)[0]
...
class DekelCore(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.phase_tap = str(config.get("phase_tap", "stage1_signed"))
        valid_phase_taps = {
            "temporal_signed",
            "stage1_pre_norm",
            "stage1_signed",
        }
        if self.phase_tap not in valid_phase_taps:
            raise ValueError(
                f"phase_tap must be one of {sorted(valid_phase_taps)}, "
                f"got {self.phase_tap!r}"
            )
    ...
    def _forward_stages_with_phase(
        self,
        x: torch.Tensor,
        *,
        ...
        if temporal.shape[2] != 1:
            raise RuntimeError(
                "The full-history convolution must collapse time to a singleton; "
                f"got temporal size {temporal.shape[2]}"
            )
        stem, temporal_signed = self.temporal_nonlinearity.forward_with_signed(
            temporal.squeeze(2)
        )

        stage1_pre_norm = self.stage1_conv(stem)
        stage1, stage1_signed = self.stage1_nonlinearity.forward_with_signed(
            stage1_pre_norm
        )
        ...
        phase_features = {
            "temporal_signed": temporal_signed,
            "stage1_pre_norm": stage1_pre_norm,
            "stage1_signed": stage1_signed,
        }[self.phase_tap]
        return (stage1, stage2, stage3), phase_features
    ...
    def forward_stages(
        ...
        *,
        ...
        """Return the three historical split-ReLU stages."""
        stages, _ = self._forward_stages_with_phase(
            x, strict_spatial=strict_spatial
        )
        return stages
    ...
    def forward_with_phase(
        ...
        """Return the established scaffold and configured phase-preserving tap.

        The scaffold is bit-for-bit the ordinary forward path.  The auxiliary
        map is 35x35 for the production aperture.  ``stage1_signed`` is the
        backwards-compatible default; earlier signed/pre-normalization taps
        let a frozen-core assay determine where phase information is lost.
        """
        stages, stage1_signed = self._forward_stages_with_phase(x)
        scaffold = torch.cat([self._to_scaffold_size(v) for v in stages], dim=1)
        return scaffold.unsqueeze(2), stage1_signed
    ...
    def forward_spatial_map_with_phase(
        self, x: torch.Tensor
    ) -> tuple[torch.Tensor, torch.Tensor]:
        """Return the translation-preserving scaffold and signed phase tap.

        The auxiliary phase branch is trained on the full 35-pixel aperture.
        For a larger image it therefore remains on the full-resolution spatial
        grid; callers slide its 35-pixel readout at the same four-pixel stride
        as the stage-3 scaffold.  Keeping that branch here prevents Figure-4
        replays from silently dropping the phase-selective logits.
        """
        stages, phase_features = self._forward_stages_with_phase(
            x, strict_spatial=False
        )
        target_size = tuple(stages[-1].shape[-2:])
        kwargs = {}
        if self.scaffold_mode in {"linear", "bilinear", "bicubic", "trilinear"}:
        ...
        return torch.cat(aligned, dim=1).unsqueeze(2), phase_features

    def get_phase_spatial_stride(self) -> int:
        """Pixel stride relating the full-resolution phase tap to stage 3."""
        return 4

    def get_output_channels(self) -> int:
        return self._final_out_channels

    def get_phase_channels(self) -> int:
        """Number of full-resolution channels exposed by ``phase_tap``."""
        if self.phase_tap == "temporal_signed":
            return self.temporal_channels
        return self.spatial_channels[0]
