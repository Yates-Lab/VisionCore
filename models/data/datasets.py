...
class DictDataset(torch.utils.data.Dataset):
    ...
    def cast(self, dtype, target_keys=None, protect_keys=None) -> 'DictDataset':
        '''
        Convert all floating point tensors to the specified dtype
        ...
        for key in target_keys:
            if protect_keys is not None and key in protect_keys:
                continue
            if isinstance(self.covariates[key], torch.Tensor) and self.covariates[key].dtype.is_floating_point:
                self.covariates[key] = self.covariates[key].to(dtype)
        return self
